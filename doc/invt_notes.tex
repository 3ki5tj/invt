...
The
  ...
  so we switched back to the double-WL (both for $c_0$ and for $c_1$, $c_2$) approach,
...
and then decays rapidly to a constant as $k$ increases
in a local sampling process.
%
We assume that the evolution along the collective-variable is Markovian
...
we can borrow Eq. \eqref{eq:lambda_Gaussian} for
$\Lambda_k$,
$$
\Lambda_k = 1 - \bar A + \bar A \exp\left( -\frac{ \pi^2 \, k^2 \, {\sigma_z^\mathrm{(samp)}}^2 } { 2 \, \Delta z^2} \right)
$$
where $\sigma_z^\mathrm{(samp)}$ is
the average move size along $z$ during sampling
and $\bar A$ is the acceptance ratio,
then
\begin{align}
  \Gamma_k
  &= 1 + 2 \, \sum_{t=1}^\infty
...
whereas $\Gamma_k$ tends to a constant $(2 - \bar A)/\bar A$ as $k$ increases.
%
