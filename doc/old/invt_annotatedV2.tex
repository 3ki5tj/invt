...
\begin{abstract}
  ...
  for the class of adaptive sampling methods including
   the Wang-Landau (WL) algorithm and metadynamics.
  %
  These sampling methods
  construct a bias potential to offset
  the potential of mean force by adaptively updating a histogram.
  %
  The updating magnitude should decrease over time
  to help reduce the noise in the bias potential,
  and the optimal schedule maximizes
  ...
  is a function of the inverse of the time or number of sampling steps.
  %
\end{abstract}
...
with a localized or multi-modal distribution.
%
Thus,
...
cancels the PMF, which is equivalent to sampling a flat distribution
...
%
These techniques employ regular updating of the bias potential
...
that are optimal for extensive sampling or long simulations,
and the modified inverse-time schedule presented is optimal for these schemes.
%
%For finite-length simulations, however,
%metadynamics may deliver better performance.
%
These results allow one to better
...
in the long time limit or extensive sampling
...
For a large system at tempertures of interest,
...
are often improved
with histogram of the sampling, $H_i$, accumulated
during simulation.
%
...
and can thus approximate the target distribution.
%
However,
%they are less well understood theoretically, as
as mentioned, the updates  break the microscopic reversibility
of the underlying sampling mechanism. Further failure to properly reduce the size of the histogram increments would lead to roughness or errors in the final estimate.
%
...
different from $1$\cite{
morozov2007, zhou2008, morozov2009}.
%
Here we will show that the ideal $c$
is unnity, but the origin of $t$ should be shifted
...
We first approximate the finite difference Eq. \eqref{eq:wl_update}
...
%
%
For the random noise part, we have
...
we  limit ourselves to $\mathbf w$'s
that satisfy
...
can be rewritten in terms of the eigenmodes of $w$
\begin{align}
  \Err
  \equiv
  \sum_{i = 1}^n \rho_i \,
...
affects the error only through the set of
$\Gamma_k$.
%
...
$\alpha(t) = \alpha_0$
for $t < 0$, and
the system has entered the sampling
...
%(Cheng: In the Hamiltonian or Newtonian version it seems that the position dependence of the mass could as easily be described by another conservative force at constant mass.)
%
...
with a similar width, $\sigma$, (cheng, the inset in figure two should be replotted to fix the dynamic range of the gray scale.)
%
\begin{equation}
  \sigma
  \approx
...
In our test system, (Cheng, we should give a  description of the test system)
...
to offset the PMF and thereby sample a flat distribution.
%
...
Our method is most useful for long simulations to achieve relatively precise answers.
%
...
of glassy systems. Our immediate interest is to apply these relations to
