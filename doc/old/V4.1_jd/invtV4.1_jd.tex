...
in computational physics and chemistry that can provide insight into
an array of phenomena not easily studied with traditional experiments.
Given a system,
often the task is to compute
a distribution, $p^*(z)$,
along a collective variable $z$ by sampling with Monte Carlo\cite{frenkel, newman, landau_binder} (MC)
or molecular dynamics\cite{frenkel, karplus2002} (MD) simulations.
The free energy, or potential of mean force (PMF), along the collective variable is proportional to $\ln p^*(z)$.
However, accurately estimating the PMF is often difficult when the collective variable is energetically restricted to local regions of the complex, multi-modal free energy surface.
Thus,
to overcome these energetic barriers and to capture the global shape of the free energy surface
an effective strategy is to introduce a bias potential that
cancels the PMF. Ideally, then, sampling yields a flat distribution
...
that build the bias potential associated with the collective variable adaptively, or on-the-fly, during simulation.
Both techniques regularly update the bias potential
...
and for convergence of the PMF
...
the precision of the final bias potential, and
...
In this study,
we present a general formalism for computing
the optimal schedule
for a any adaptive FDS method,
...
We show that the optimal schedule
...
turns out to be optimal for these schemes as well.
These results may help to better
plan and optimize FDS simulations, which, at present, is done heuristically and through trial-and-error.
...
biased sampling that generates a
a wider, smoother target distribution, $p_i$.
...
to converge to the target distribution, $p_i$,
which is usually chosen to be flat
(i.e. $p_i$ constant for all $i$'s).\cite{
...
uses a fixed bias potential, $V_i$ (estimated a priori),
during simulation.
...
continuous, rapid improvement of the bias potential, and subsequently the PMF,
as simulation lengthens.

The second type of methods,
referred herein as adaptive FDS methods,
address the above shortcomings
by continuously and incrementally updating the bias potential to accelerate convergence of the target distribution.
Examples include
...
many adaptive FDS methods adopt a schedule to
...
in improving the precision of the bias potential, thus
allowing for maximal efficiency
...
the distribution, $\pi_i$, collected from
an adaptive FDS simulation
is identical to $p_i$ in the long time limit.
However, as the above updates
change $V_i(t)$ continuously,
the equilibrium distribution ()Eq. \eqref{eq:pi_p_V}) no longer holds
at all times,
...
for a general, universal updating scheme.
...
This is done by removing two contributions from the bias potential.
...
In this way,
the variable, $x_i(t)$,
represents the error of the bias potential,
and is expected to vanish on convergence.
We, therefore, define
...
sharing the same schedule $\alpha(t)$ and
...
in terms of asymptotic convergence.
...
\end{equation}
The optimal schedule for these types of schemes
is given by the inverse-time scheme (Eq. \eqref{eq:alpha_invt1}).
...
\begin{center}
  ...
  \caption{
    ...
    Inset. Updating matrix associated with the non-periodic case.
  }
\end{center}
\end{figure}
...
\end{equation}
The kernel in the non-periodic case is sawtooth
...
is canceled by the terms associated with $l = i + j - 1$
or $l = i + j - 2 \, n - 1$
with opposite sign.
...
In the above section, we have focused on
...
\label{eq:t0_sinc}
\end{equation}
From Eqs. \eqref{eq:error_split}, the total error is
\eqref{eq:error_res1},
\eqref{eq:tk0},
and
...
Using the model system above, we tested our analysis of computing errors
...
and the triple-bin (bandpass) updating scheme with $\mu_1 = 0.24$.
...
but not necessarily so for a general, multi-bin updating scheme.
...
we could modify the optimal schedule
(Eq. \eqref{eq:q_opt})
and limit the sum
...
However, if certain eigenmodes associated with local noise
are expected to vanish in the PMF,
we can construct a
...
can be ignored as a uniform shift that
...
To do so, let us consider moving simulation by $\delta T$ time from
...
defined in Eq. \eqref{eq:mass_func}.
%
