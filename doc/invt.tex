...
corresponding to the longest range motion, as $\Gamma_1 \approx \Gamma$,
and the suppression of local noise
had little effect on reducing the total error.
%
...
To further investigate the behavior of the Gaussian updating scheme,
...
%
In the long time limit, the fluctuation of
...
we conclude
\begin{equation}
  \langle F(T+t_0) \rangle \approx E(T)
  .
  \notag
  %\label{eq:fluc}
\end{equation}
...
in initial stages, with a modified definiton
for the histogram fluctuation
$$
F_{<K}(T) = \sum_{k=1}^{K-1} \tilde \Xi_k^2(T),
$$
since the histogram-flatten
...
\Gamma_k = \coth\left[
  ...
  \right].
$$
For small $k$, we have $\Gamma_k \propto \Delta z^2/(\sigma_z \, k)^2$,
