...
by incremental elevation of the bias potential.
%
...
can benefit from a gradually decreasing
...
by solving Eq. \eqref{eq:opt_qT}.\cite{*[{We
  ...
  $\bigl|\eta\bigl(q(T)\bigr)/q(T)\bigr|$, for
  $\eta'\bigl(q(T)\bigr)$
...
that is more effective in error control
...
As shown below,
the optimized updating scheme will spare the need to
go through the general procedure in Sec. \ref{sec:procedure}
because the optimal schedule is given by
...
$\Err_A(T)$, as defined in Eq. \eqref{eq:error_asym}
...
\begin{align}
  ...
  \left(
    \int_0^T \!\!dt
      \sum_{k = 1}^{n-1}
        \Gamma_k \, {\dot \vartheta}_k \, \bigl( q(t) \bigr)
  \right)^{\!\!2}
  \notag \\
  & \qquad \qquad \qquad
  =
...
if $\dot \vartheta_k\left( q(t) \right) = \mathrm{constant}$
...
\begin{align*}
  ...
  ,
\end{align*}
which is essentially the diagonal updating matrix
that represents the single-bin updating scheme.
%
...
as the former expresses the bias potential as
...
of the discrete umbrella index, $s = 1, \dots, S$,
...
an independent bias potential, $u^{(s)}(z)$,
...
and it is updated according to the single-bin updating scheme.\cite{li2007}
%
With a small width, $\sigma_z$, the converged
$c_0^{(s)}$ as a function of $z_c^{(s)}$
...
\item
  Set up an array of Gaussian umbrellas covering the desired range,
    ...
    and
    the width, $\sigma_z^{(s)}$, to be a constant.
    For the polynomial coefficients, we can set
    $c_1^{(s)}/\sigma_z^{(s)}$ to the estimated mean force,
    and $c_0^{(s)} = c_2^{(s)} = 0$.
...
\item
  ...
  in the current umbrella $s$ under bias potential $u^{(s)}(z, t)$.
...
umbrellas were performed.\footnote{For
  wider umbrellas,
  one may consider
  mixing the nearest-neighbor inter-umbrella transitions
...
by $c_0^{(s)}$ as a function of $E_c^{(s)}$,
...
to early stages of a simulation
and we still have to resort to the WL stage-by-stage
technique even for the bandpass updating schemes.
%the random error,
...
%
However, this is only an approximate result,
and it does not remove the systematic bias
...
and the updating magnitude is larger in the former case.
...
We will use Eq. \eqref{eq:xt2_eql}
but modify it to avoid the unknown
intrinsic distribution, $p_i$,
hence the unknown shift,
...
%\end{equation*}
%
Then, from Eq. \eqref{eq:xt2_eql}, we get
%
\begin{equation}
...
would fail for a near zero eigenvalue.
