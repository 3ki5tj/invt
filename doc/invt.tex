...
\begin{abstract}
  ...
  of the updating magnitude.
  %
  Here, we derive the optimal schedule from
...
%
The difference lies
in the updating hill function,
which takes the form of a localized
$\delta$-function in the WL case,
...
the updating magnitude,
therefore, is critical in improving
...
and the above origin-modified inverse-time schedule
...
%
These results may hopefully help one to better
...
are often improved
with the histogram of the sampling, $H_i$, accumulated
during simulation.
%
...
The bias potential, $v_i(t)$, following Eq. \eqref{eq:vt_diffeq}
generally grows overtime.
%
While a uniform increase of $v_i(t)$ does not affect the 
resulting distribution $\pi_i(t)$ as given by Eq. \eqref{eq:h_ave},
%
it is convenient to express the error of $v_i(t)$,
...
%
The rate of growth of $v(t)$ is given by
\begin{equation}
  \frac{ d \bar v (t) }
       { d t }
  =
...
%
\note{
The second step of Eq. \eqref{eq:hdet_linearize} follows
$$
...
can be characterized by a hill function,
which is a rigid window
...
$p^*_i = p_i = 1/n$,
so is the distribution $\rho_i$ used in Eqs.
...
including the Wang-Landau (WL) algorithm or metadynamics.
%
...
The optimal schedule depends on the updating hill function,
...
hill function for the bin corresponding to the column.
%
...
Our method is most useful for long simulations
...
%
We assume that
the magnitude decreases geometrically,
%
\begin{equation}
  \alpha_s = a \, r^s
  ,
...
$q(t) = \alpha_s \, t$.
%
Our aim is to minimize $E_S$
of the final stage.
%
To do so, consider moving $\delta T$ simulation time from
...
,
$$
where we have used Eq. \eqref{eq:exp_r} in the last step.
%
Thus,
%
\begin{equation}
\alpha_s = \frac{1}{
  \frac{1 -r }{ \ln(1/r) } t + \frac{1}{a}
},
\label{eq:alphas}
\end{equation}
%
Thus, the final error is given by
%
\begin{align}
  E_S
  =
...
\end{align}
%
which is minimized at $r \to 1$,
for a large $T$.
%
If we identify the parameter $a$ as the initial updating magnitude $\alpha(t = 0)$,
then this result agrees with Eqs. \eqref{eq:half_alpha0} and \eqref{eq:t0_sinc},
and the optimal $t_0 = 1/a$.
%
Thus, in this limit,
the schedule given by Eq. \eqref{eq:alphas}
...
and hence
cannot compute the shifted bias potential, $v_i$,
...
\note{The table of symbols is listed in Table \ref{tab:symbols}.
  ...
  \begin{tabular}{l | p{12cm} }
    ...
    $\nu_k$         &   $\nu_k \equiv \lambda_k / \lambda$. \\
    $p_i$           &   Distribution. \\
    $\pi_i$         &   (Instantaneous) distribution, Eq. \eqref{eq:pi_p_phi}. \\
    ...
    $\chi_i$        &   $\chi_i \equiv \ln \frac{ p_i } { p^*_i } - \sum_{ j = 1 }^n p_j \, \ln \frac{ p_j } { p^*_j }$. \\
