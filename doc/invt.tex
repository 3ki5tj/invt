...
and chosen for convenience.\footnote{In
...
Our first test system is a Lennard-Jones fluid of $N=100$ particles,
...
but interact normally with other particles
(they have placed on an axis parallel to a side of the cubic box),
and the intrinsic distribution, $p^*$, is often referred to as
the cavity distribution function, $y(r)$.
%
We will assume reduced units in this calculation
%
The reference values were pre-calculated from a long simulation.
%
We used the Metropolis algorithm for configuration sampling\cite{frenkel}.
%
Each step consists of five MC trials of displacing a random particle,
one for one of the two special particles,
and the rest four for the other particles.
%
The MC displacement along each dimension is uniform,
...
%
Each simulation had an ``equilibration'' phase
...
%
Each simulation was repeated multiple times
and the average are reported.
%
For multiple runs under the same condition,
...
collected from the equilibration phase
...
\begin{table}[h]\footnotesize
  \caption{\label{tab:error_LJ}
    ...
    The numbers in parentheses are predicted values;
    $\Err_\mathrm{init.}$
    denotes the error
...
and this correction is absorbed into the new value, $X(t)$,
...
%
Since the normalized histogram, $H_i(t)$,
is an average of the instantaneous histogram, $h_i(t)$,
$H_i(t) = \frac 1 t \sum_{\tau = 1}^t h_i(\tau)$,
we have, from Eq. \eqref{eq:avx_recur},
%
\begin{align}
  H_i(t)
  &=
...
in the asymptotic limit,
Eq. \eqref{eq:vhat_update} recovers
...
and one with a fixed and nearly perfect bias potential.
%
Since the bias potential in the former approach
essentially follows the same time evolution
for the correction,
$\hat u_i(t)$, Eq. \eqref{eq:vhat_update},
the two would reach similar precision at long times.
%
