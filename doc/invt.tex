...
we have, from Eq. \eqref{eq:xt_solution} [with $T\to T_0, q(t) \to a_0 \, t$],
%
\begin{align}
...
to cover a wide range of the collective variable, $z$.
%
...
which is common in umbrella sampling
...
\item
  Set up an array of Gaussian umbrellas covering the desired range,
    ...
    $c_1^{(s)}/\sigma_z^{(s)}$ to some estimated mean force, $f_z$,
    $c_0^{(s)} = f_z \bigl[ z_c^{(s)} - \frac{1}{2}(z_{\min} + z_{\max}) \bigr]$
...
%\end{equation*}
%
Then, from Eq. \eqref{eq:xtvar}, we get
%
\begin{equation}
...
%
In practice, Eq. \eqref{eq:varXt} would fail for a near zero eigenvalue.
%
