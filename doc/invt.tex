...
%
While a slow reduction leads to large variation of the bias potential,
...
%
Further, we will compare
the highest efficiency of error reduction
...
and to filter local noise
during the updating process.

The bandpass updating schemes are convenient
in that their optimal schedule is simply given by
the inverse-time formula,
and thus requires no extra calculation.
%
They can be constructed from any orthogonal basis\cite{bussi2006},
and for a polynomial basis,
this construction results in
a class of adaptive FDS algorithms that
update the bias potential based on the
statistical moments of observed distribution
as a generalization of the Langfeld-Lucini-Rago (LLR)
algorithm\cite{langfeld2012, pellegrini2014}.
%
We show that a similar construction
can help FDS algorithms with
quadratic bias potentials\cite{neuhaus2006, *neuhaus2007, zhu2012}
automatically adjust the parameters
to reach a set of Gaussian distributions
...
that based on the statistical moments.
...
Sampling a multimodal distribution with deep valley(s)
is often challenging in traditional simulations,
and a common remedy is to apply a bias potential
to sample a smoother distribution, $\rho_i$.
%
...
\end{equation}
%
our aim is to find a bias potential that allows
...
is an example, with $w_{ij}$ being
roughly a Gaussian centered on bin $j$.
%
...
%
If we assume that at the beginning, $t = 0$,
the system has gone through a long simulation
...
We will first study the eigenvalue decomposition
of the updating matrix, $\mathbf w$,
...
we will assume that
the system undergoes a preliminary FDS simulation
under a constant updating magnitude, $a_0$,
for $T_0$ steps.
%
...
Perhaps the most commonly-used multiple-bin updating scheme in metadynamics
...
to satisfy the constraints discussed
...
\begin{center}
  ...
  \caption{
    ...
    %
    Both updating schemes are homogeneous in that
    they can be characterized a rigid window
    centered on the current bin, $j$,
    with some boundary corrections.
  }
\end{center}
\end{figure}
...
that can be characterized by a rigid window function
with a floating center.
%
These homogeneous updating schemes share
the same set of orthonormal eigenmodes, $\phi_{ki}$,
(which are sines and cosines),
and differ only by the eigenvalues.
%
By the completeness of the eigenmodes,
we can readily reconstruct the updating matrix
as\cite{bussi2006}
%
\begin{equation}
  w_{ij}
...
\end{equation}
%
This formula allows us to define new updating schemes
whose spectrum of eigenvalues are optimal
for asymptotic convergence,
which we will discuss below.
%
The result is a class of bandpass updating schemes,
...
that completely discard certain noise modes
...
eigenmodes,\footnote{As an estimate of error,
...
\end{equation}
%
We can then obtain the updating matrix
from the eigenvalues by Eq. \eqref{eq:w_from_phi}
(cf. Sec. \ref{sec:homo_bandpass} for the case
of homogeneous updating schemes).
%
...
that completely removes the components of the noise modes
from the potential of mean force.
%
...
For homogeneous updating schemes,
the bandpass updating scheme resembles Gaussian updating schemes,
but contains long-range wiggles,
as shown in Fig. \ref{fig:mat}(b).
%
Both updating scheme are able to deliver
a smooth and differentiable potential of mean force,
the former is expected to be superior in asymptotic convergence,
%
With a $0$-or-$1$ spectrum of eigenvalues,
Eq. \eqref{eq:lambda_bandpass},
instead of a Gaussian one, Eq. \eqref{eq:lambda_Gaussian},
the bandpass updating scheme has the advantage
of allowing convenient control of the updating magnitude.
%
In the asymptotic regime,
the optimal schedule is simply given
by the inverse-time schedule, Eq. \eqref{eq:alpha_invt1},
%with $t_0$ given by Eq. \eqref{eq:t0_sbin},
just like the single-bin scheme,
and it is therefore unnecessary to go through
the procedure in Sec. \ref{sec:procedure}
for a general updating scheme.
%
In initial stages, when the inverse-time schedule
may be inapplicable,
we can instead use the WL style stage-wise control
for the updating magnitude.
%
In this case, the histogram fluctuation used for stage transitions
...
Bandpass updating schemes constructed from polynomial bases}

\subsubsection{Orthogonal polynomial basis}

We can also construct the bandpass updating schemes
from an orthogonal polynomial basis,
which can simplify the functional form of the bias potential.
%
In discussing this application,
it is more convenient to switch to a continuous-variable setup:
%
We will replace the discrete bin index by the continuous variable, $z$.
%
and map $\rho_i$ to $\rho(z) \, \delta z$,
  $\phi_{ki}$ to $\phi_k(z) \, \delta z$,
  $w_{ij}$ to $w(z, z') \, \delta z$, etc.
%
We define the eigenvectors by
$\phi_k(z) = R_k(z) \, \rho(z)$,
where $R_k(z)$ are a set of orthogonal polynomials
satisfying
\begin{align*}
...
\end{align*}
%
These eigenvectors allow us to construct the updating matrix
Eq. \eqref{eq:w_from_phi},
...
\subsubsection{Adaptive Gaussian umbrella sampling}
...
$Z \equiv \int e^{-\beta \, \Phi(\mathbf x)-u(z(\mathbf x),t)} \, d\mathbf x$
...
via the pair potential $u(r) = 4(r^{-12} - r^{-6})$.
%
...
\begin{align*}
  ...
  =
  \frac{V}{Z}
  \int e^{-\Phi(\mathbf r^N)/(k_B T)}
  d\mathbf r_3 \cdots d\mathbf r_N
  ,
\end{align*}
where
$\Phi(\mathbf r^{N}) = \sum_{j=3}^N\sum_{i=1}^{j-1} \phi(r_{ij})$,
%
and it is related the cavity distribution function, $y(r)$\cite{hansen},
as $y(r) = \left(1 - \frac{1}{N}\right) \, V \, p(r)$.
%
...
which would ideally bias $p(r)$
to a flat distribution along $r$.
%
...
with width $\sigma = 0.2$,
if we increase the simulation length to $10^8$ steps,
...
\begin{figure}[h]
\begin{center}
  \begin{tikzpicture}[thick, scale=1.4, every node/.style={scale=0.9}]
    \node [] at (-0.3, 2.0) {(a)};
...
(excluding the boundary adjustment).
%
...
these schemes satisfy detailed balance,
...
\end{equation}
%
where $g = 1$ here, with a two-fold degeneracy,
