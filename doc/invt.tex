...
\begin{abstract}
  ...
  and attempt to give similar prescriptions for the metadynamics case.
  %
  Our results show that the optimal schedule
...
while metadynamics adopts an extended updating window
...
%
Thus, one often has to reduce the magnitude of updating
over the course of simulation.



Naturally, the updating schedule,
...
In this study,
we shall reexamine the inverse-time formula,
and similarly optimize schedules
for updating schemes involving several neighboring bins,
such as those used in metadynamics.
%
It turns out that the optimal updating schedule
can be sensitive the simulation length.
%
...
depends on the schedule $\alpha(\tau)$
only through the integral $q(t)$.
%
...
for an arbitrary updating matrix $\mathbf w$
often leads to a complicated expression for $\alpha(t)$.
%
In practice, it is also useful to find
a reasonably optimal $\alpha(t)$ with a simpler expression.
%
From Eq. \eqref{eq:alpha_opt},
we consider the following invert-time formula
%
\begin{equation}
...
We now consider a class of commonly-used updating scheme
for sampling a uniform distribution, $p_i = 1/n$.
%
These schemes are homogeneous,
or translational invariant with respect to $z$.
%
This invariance is natural for a periodic variable\cite{dama2014}.
%
Below, however, we focus on a $z$ that is
under the reflective boundary conditions.
...
The updating scheme determined by an arbitrary set of numbers,
$\mu_1, \dots, \mu_b$, is not necessarily stable,
i.e., the eigenvalues from Eq. \eqref{eq:wband_eigenvalue}
can be negative.
%
Below we show how to minimally modify these numbers
to make the eigenvalues nonnegative.

The key of this modification is the inversion formula
%
\begin{align}
  \mu_l
  =
...
\end{align}
%
which ensures that the $\mu_n$
computed from Eq. \eqref{eq:mu_from_lambda}
vanishes.
%
\note{The inversion formula is derived from the cosine transform.
  %
...
Thus, the algorithm of stabilization consists of three steps.
%
\begin{enumerate}
  \item
    Compute the eigenvalues from $\lambda_0, \dots, \lambda_{n-1}$
    from the given $\mu_0, \dots, \mu_b$ with $b < n$
    according to Eq. \eqref{eq:wband_eigenvalue}.
  \item
    Replace the negative eigenvalues by zeros.
  \item
    Compute the new $\mu_l$ $(0 \le l < n)$ from
    the modified eigenvalues according to
    Eq. \eqref{eq:mu_from_lambda}.
\end{enumerate}
%
Note that, after the stabilization,
the window is usually widened.
...
\begin{center}
  \makebox[\linewidth][c]{
    \includegraphics[angle=0, width=0.95\linewidth]{fig/alphacmp.pdf}
  }
  \caption{
    \label{fig:alpha_cmp}
    Optimal schedules, $\alpha(t)$, from Eq. \eqref{eq:q_opt},
    versus the inverse-time schedule,
    Eq. \eqref{eq:alpha_invtlambda},
    with the optimal parameter, $\lambda$.
    %
    For fair comparison,
    the $q(t) = \int_0^t \alpha(\tau) \, d\tau$
