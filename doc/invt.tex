...
%
The bandpass updating schemes allow
simpler control of the updating magnitude
than the Gaussian updating scheme
for the asymptotically optimal schedule of the former
...
a polynomial form,
and coefficients adjusted on-the-fly
to achieve the desired target distribution.
%
This method serves
...
hence its deviation from the bin average, $\xi_{*i}(t)$,
...
eigenmodes,
we may assign the first $K$ eigenmodes eigenvalue $1$
and the rest of eigenmodes eigenvalue $0$:
%
\begin{equation}
...
We will consider two special cases of the bandpass updating schemes.
%
First,
...
%
The second case is based on a family of orthogonal polynomials,
...
Here we will consider here a family of bandpass updating schemes
...
%
The parameter, $Z^{(i)}$, is the inverse weight of the $i$th umbrella,
in the expanded ensemble,
and $\ln Z^{(i)}$ serves as the bias potential
for inter-umbrella transitions.
%
We can be estimate $\ln Z^{(i)}$'s on-the-fly
using the single-bin updating scheme,
Eq. \eqref{eq:wl_update}\cite{li2007}.
%
Upon convergence, $Z^{(i)}$ should be proportional to
...
without going through the usual WHAM iteration.
...
%
However, the two special particles are fixed on the $x$-axis,
...
both the optimal and sometimes sub-optimal inverse-time schedules
were able to reduce the error significantly.
%
It is also reassuring that
...
and the former tended to give larger updating magnitude.
%
...
For the same case,
we further decomposed the error (after $10^7$ production steps)
into the eigenmodes
...
(the deviation from a perfectly flat profile
...
Next, we will discuss the applicability of
...
also induce a huge initial updating magnitude
...
becomes invalid the resulting deviation in the bias potential
is no longer small.
...
%
In Fig. \ref{fig:lj_xerr},
we show that the correction produced
...
the bias potential no longer need to be differentiated
