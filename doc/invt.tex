...
$c_1(t)$ represents the scaled local or
...
With only two nontrivial modes,
the above strategy usually only works
for a Gaussian with a narrow width.
%
We can, however, use it as a building block
to construct an expanded ensemble with
multiple Gaussians centered at different places.
%
Such a construction can be realized via
either the replica exchange
(parallel tempering)\cite{swendsen1986,
...
we can estimate the ratio $Z^{(i)}/Z^{(j)}$
