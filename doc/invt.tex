...
%
This is equivalent to varying the curve,
$\vartheta\bigl( q(t) \bigr)$,
...
that completely discard certain short-wavelength eigenmodes
that are deemed as local noise.
%
...
we may construct an updating scheme
from Eq. \eqref{eq:w_from_phi} with
%
\begin{equation}
...
%%
With the eigenvalues for the non-noise and noise modes being
...
Comparing Eq. \eqref{eq:lambda_bandpass}
with Eq. \eqref{eq:lambda_Gaussian},
we find that the bandpass updating scheme serves
as an idealization of the Gaussian updating scheme
with the Gaussian spectrum of eigenvalues
being replaced by a step function.
%
Although both updating schemes are able to remove local noise
from the bias potential,
the bandpass updating schemes have the advantage
...
may be inapplicable,
we can instead use the stage-wise WL technique
as long as the histogram fluctuation
used for stage transitions
is computed from the first $K$ modes
(cf. Appendix \ref{sec:hfluc}).
