...
%
In this way, the bias potential is a superposition of the polynomials
$$
u(z, t) = \sum_{k=1}^{K-1} a_k(t) \, R_k(z),
$$
with the coefficients updated as, according to Eq. \eqref{eq:mbin_update},
$$
a_k(t+1) = a_k(t) + \alpha(t) \, R_k\bigl( z(t) \bigr).
$$


For example, by assuming the Legendre polynomials\cite{arfken}
on a flat distribution over $[z_c - \Delta z/2, z_c + \Delta z/2]$
and $K = 2$,
the bias potential contains only a linear mode for
$R_1(z) = 2 \sqrt{3} (z - z_c)/\Delta z$,
and we recover the algorithm by
Langfeld, Lucini and Rago\cite{langfeld2012}.
%
Applied to the energy space, $z = E$,
$a_1(t)$ represents the local or
...
Alternatively, by assuming the Hermite polynomials\cite{arfken}
on a Gaussian distribution centered at $z_c$ with width $\sigma_z$
and $K=3$,
we have a quadratic bias potential
as a linear combination of
$R_1(z) = (z-z_c)/\sigma_z$
and
$R_2(z) = (z - z_c)^2 /\left(\sqrt 2 \, \sigma_z^2\right)$.
%
...
which is helpful in overcoming the barrier
in a first-order phase transition.
%
Initially, we can employ the WL strategy
to control the updating magnitude
with the histogram fluctuation redefined as
$$
\max\left\{
  \left|
    \frac{ \overline z - z_c } { \sigma_z }
  \right|
  ,
  \left|
    \frac{ \overline{ (z - z_c)^2 } - \sigma_z^2 }
    { \sqrt 2 \, \sigma_z^2 }
  \right|
  \right\}
  ,
...
We wish to show that with the inverse-time schedule,
the bias potential under the single-bin updating scheme
happens to coincide with a runtime average
