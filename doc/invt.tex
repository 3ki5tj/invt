...
\begin{abstract}
  ...
  (such as the single-bin scheme commonly used in WL
  or the Gaussian one popular in metadynamics),
...
we will loosely use the terms WL and metadynamics
...
\end{equation}
%
where the modified histogram fluctuation
%
\begin{equation}
  \xi_i(t)
  =
...
%
This artificial condition
restrains the form of the updating matrix, $\mathbf w$,
and it requires, for the commonly-used uniform distribution, $\pmb\rho$,
a symmetric updating matrix, $\pmb w$.
%
More generally, only
...
\subsubsection{Error functional}
...
\end{align}
%
where we have approximated $\theta_k(t)$
as white noise:
%
\begin{equation}
  \kappa_k(t)
  \equiv \langle \theta_k(0) \, \theta_k(t) \rangle
  \approx \Gamma_k \, \delta(t)
  .
  \notag
...
For the theory to be applicable to
a general sampling process,
we can map the histogram fluctuation
to the equivalent white noise, with
\begin{equation}
  \Gamma_k
...
from eigenmodes corresponding to long-range fluctation
in a local sampling process,
especially for complex or glassy systems.
%
...
We will minimize the error under a fixed value of $q(T)$.
%
This allows us to focus on the asymptotic part
...
\end{equation}
%
where the $g = 1$ for the periodic case,
or $2$ otherwise.
%
In both cases,
...
%
Note also that
the updating scheme should be stabilized
...
%
We start from an interval $[Q_L, Q_R]$
that satisfies $m(Q_L) > 2/(T\,a_0) > m(Q_R)$
(e.g. we can choose $Q_L = 0$,
  and repeatedly increase $Q_R$ until
...
the result from the single-bin scheme,
Eq. \eqref{eq:Emin_singlebin}
[one can show that $\sum_{k=2}^n \Gamma_k = \sum_{i=1}^n \rho_i \, G_i = \Gamma$,
following the steps similar to those leading to Eq. \eqref{eq:y2_sum}].
%
\note{The derivation.
...
and are even functions about the axis, $i = 1/2$:
%
\begin{equation}
  \phi^{(k)}_i
  =
...
is the bin size.
%
By assuming the largest cutoff, $b \approx g \, n/2$,
...
\end{align*}
The upper limit,
$b \, \Delta x \approx (g\,n/2) \, [2\,\pi/(g\,n)] = \pi$.
}
...
in Eq. \eqref{eq:error_asym} yields
