...
Eq. \eqref{eq:mbin_update} [using $u_i(t)$ instead of $v_i(t)$],
as
%
\begin{equation}
  {\tilde v}_k(t + 1) =
...
%
In this expanded ensemble setup,
the updating equation for $c_0$ is no longer trivial,
but corresponds to the WL way of constructing the bias potential
for transitions among different intervals.
%
The WL algorithm is in fact the $K = 1$ case.
%
...
we used a set of fixed reference values of $\Gamma_k$'s\footnote{We
  derived the reference $\Gamma_k$'s
...
there were considerable derivations
...
the WL and LLR\cite{langfeld2012, pellegrini2014} algorithm,
...
(possibly because of the deviation from
the quasi-equilibrium sampling).


Alternatively, we can estimate $\Gamma_k$ using Eq. \eqref{eq:Fk2_Gammak}.
%
By splitting the trajectory into blocks of $T_\mathrm{blk}$ steps,
...
when there are dynamic updates to the bias potential
