...
can critically affect the precision of the final bias potential,
...
increases the variance of the bias potential,
a too rapid reduction often
converges the bias potential to wrong value.
%
...
%
The same asymptotic optimality also applies to
...
can be mapped to a runtime average computed from all data points collected so far;
...
%
We then compute the updating magnitude
...
a fixed Gaussian centered on the current bin
...
are the width of the Gaussian and the range of
the original collective variable, $z$,
respectively.

More generally, we can replace the Gaussian
in the above updating scheme
by a window function of different shape.
%
This results in a
translationally-invariant or \emph{homogeneous} updating scheme,
...
we turn to the problem of finding
optimal updating schemes
assuming that each updating scheme
...
%
The second class of bandpass updating schemes
is constructed from a sequence of orthogonal polynomials,
...
this method is better used with a small $K$ for a narrow distribution.
%
However, by constructing
an expanded ensemble or mixture\cite{swendsen1986,
...
$\rho_i = 1/m$,
whereas the conditional intra-umbrella distribution, $\rho^{(i)}(z)$
is the Gaussian given previously.
%
To sample this distribution, we give each umbrella
an independent bias potential, $u^{(i)}(z)$,
of the form of Eq. \eqref{eq:uz_decomp},
and update coefficients as
\begin{align*}
  c_k^{(i)}(t+1) = c_k^{(i)}(t)
  + m \, \alpha(t) \, R_k^{(i)}\bigl( z(t) \bigr) \, \delta_{i, i(t)}
  ,
\end{align*}
%
where the factor $m$ before $\alpha(t)$
is due to the inverse of the marginal inter-umbrella distribution, $\rho_i$.
%
Note that since $R_0^{(i)}(z) = 1$,
the parameter, $c_0^{(i)}$,
...
%
%and upon convergence, $Z^{(i)}$ should be proportional to
%the partition function,
%$\int e^{-\beta \, \Phi(\mathbf x)-u^{(i)}(z(\mathbf x),t)} \, d\mathbf x$,
...
once the updating magnitude from the WL approach
...
\item
  ...
  Update the coefficients, $c_k^{(i)}$'s,
...
in umbrella $i$,
and the $N^{(i)}$ is the total number of visits
to the umbrella $i$.
%
We can use the final converged values of $Z^{(i)}$'s
without going through the usual WHAM iterations.
...
We used adaptive FDS to sample a flat distribution along $r$,
and calculated the PMF from $r = 0$ to half of the box size
%(also the cutoff distance of the potential)
on a grid of size $\delta r = 0.01$ in reduced units.
%
Two densities were used, $\varrho = 0.1$ and $0.8$,
and the numbers of bins were $n = 500$ and $250$, respectively.
%
The reduced temperature was $T = 3$.
%
The bias potential, $k_B T \, u(r)$,
...
another $10^7$ steps (unless indicated otherwise)
...
%
To ensure the same schedule was used for multiple runs under the same condition,
...
%
Since the two schedules are identical
in the single-bin and bandpass updating schemes,
we will focus on the Gaussian updating scheme below
...
the optimal schedules for the Gaussian updating schemes
differed from inverse-time one,
and the former tended to give a larger updating magnitude.
%
The optimal schedule was more effective
than the sub-optimal inverse-time schedule
in reducing the error,
as shown in Table \eqref{tab:lj_error}.
%
Although the advantage of the former
was modest in the reported cases,
it would increase with the simulation length.
%
For example, for the case of
$\varrho = 0.1$ and Gaussian width $\sigma = 0.2$,
...
For the same case,
we further decomposed the error
into the eigenmodes
by Eqs. \eqref{eq:gft_def} and \eqref{eq:wband_eigenvector_refl},
and ploted the error components, $\langle \tilde v_k^2 \rangle$,
in Fig. \ref{fig:lj_xerr}.
%
We found that the inverse-time schedule tended to
reduce the updating magnitude too quickly,
leaving a hump on the error profile
for the $k = 10$ to $25$ modes.
%
...
was likely due to the error of estimated values of $\Gamma_k$'s).
...
Next, we discuss the applicability of
the Gaussian updating scheme.
%
As shown in Table \ref{tab:lj_error},
the Gaussian updating schemes,
or more generally the bandpass updating schemes,
produced less error than the single-bin scheme
only in the low-density $\varrho = 0.1$ case,
but not in the high-density $\varrho = 0.8$ case.
%
We believe that the difference lies in the sampling difficulty.
%
In the low-density case,
the sampling along the collective variable
was free from major barriers,
and thus the histogram fluctuation
was largely local in nature.
%
In the high-density case, however,
the error was dominated by the first few eigenmodes
that correspond to the long-range fluctuations
(as $\Gamma_1 \approx \Gamma$),
and the reduction of local noise by multiple-bin schemes
had little effect on the total error.
%
When local noise is dominant,
we found that the bandpass updating scheme
(with the mode cutoff $K = 20$)
was more effective than the Gaussian updating schemes
as expected.



It is worth mentioning that the width $\sigma$
of a Gaussian updating scheme
can critially affect its effectiveness.
%
With a overly wide Gaussian,
the updating scheme would fail to
capture finer features of the PMF.
%
From Eq. \eqref{eq:lambda_Gaussian},
we know that increasing the width of the Gaussian
would diminish the eigenvalue, $\lambda_k$,
which is proportional to the rate of error reduction
by Eq. \eqref{eq:vt_diffeq_mbin}.
%
Thus, although widening the Gaussian can
effectively suppress random error
from local histogram fluctuations,
it also increased the risk of leaving
more persistent bias in the middle-range modes.
%
For the $\varrho = 0.1$ case,
the prediction showed that
assuming the optimal schedule,
the optimal width was around $\sigma = 0.37$,
and it would decrease with the simulation lengthen.
%
Similarly, the optimal mode cutoff, $K$,
for the homogeneous bandpass updating scheme
would increase with the simulation length.
%
For very long simulations, we expect both to reach
the limit of single-bin updating scheme
($\sigma \to 0$ or $K\to n$).

An overly large width $\sigma$ can
also induce a very large initial updating magnitude
...
for a sufficiently long preliminary run
under the constant updating magnitude, $a_0$,
as shown in Fig. \ref{fig:lj_alpha}.
%
But if the Gaussian were too wide,
e.g. if $\sigma = 0.6$ in the $\varrho = 0.8$ case,
the eigenvalues $\lambda_k$ of some modes
became too small to damp out the initial bias,
resulting in a much larger $\alpha(0)$
[cf. Eq. \eqref{eq:opt_alpha0}].
%
In this case, the predicted optimal schedule
becomes invalid as the resulting deviation in the bias potential
is no longer small.
...
In practice, both the width and schedule
of the Gaussian updating scheme are often sub-optimal.
%
We can use the histogram-based correction formula,
Eq. \eqref{eq:uhatav}, after simulation
to remove the remaining bias in PMF.
%
In Fig. \ref{fig:lj_xerr},
...
%
However, this correction also introduces
some local noise in the large $k$ modes,
although these noise are largely harmless
in a post-simulation analysis, since
...
\begin{figure}[h]\centering
  ...
  \caption{
    ...
    manifested the backbending behavior
    typical in a first-order phase transition.
    %
...
of glassy systems.
%
Finally, the adaptive umbrella sampling method
introduced in Sec. \ref{sec:aus}
requires refinement to improve
...
Conversely, for a recurrence relation
in the form of Eq. \eqref{eq:X_recur}
with a given $\alpha(t)$, we can revert it
...
For example,
the unweighted average, $\omega(t) = \omega(0)/t_0$ (for $t > 0$),
corresponds to the inverse-time updating magnitude,
Eq. \eqref{eq:alpha_invt1}.
%
Updating magnitudes above and below this value
correspond to runtime averages
with growing and diminishing weights, respectively.
...
we get an independent estimate of the bias potential
made by a single data point at step $t$,
%
\begin{equation}
  \hat u_i(t)
...
we find that $u_i(t+1)$
is equivalent to a runtime average $X_\omega(t)$
of $\hat u_i(t)$
with the weight given by Eq. \eqref{eq:omega_from_alpha}.
%
The inverse-time schedule is optimal
because it corresponds to the unweighted average
that uses the data most efficiently.

Similarly, for the multiple-bin updating scheme,
we have, from Eq. \eqref{eq:vkupdate},
...
for $k > 0$.
This means that
$\tilde u_k(t+1)$ can also be cast
as a runtime average of $\hat{\tilde u}_k(t)$,
with a weight corresponding to the effective updating magnitude,
$\lambda_k \, \alpha(t)$ [instead of $\alpha(t)$].

\subsection{Upper bound of the updating magnitude}

The runtime average interpretation imposes
...
would guarantee $\lambda_k \, \alpha(t) \ge 1$
...
corresponds to a runtime average
with a dynamic weight,
which is not necessarily the optimal even weight.
%
Below we give a simple correction that
could largely compensate the efficiency loss.
%
We will use capital letters to denote unweighted averages.
%e.g. for variable $y$,
...
%\end{equation}
%
Then, by an unweighted average of Eq. \eqref{eq:uhatt}, we get
\begin{align}
  \hat U_i(t)
  &\approx
  U_i (t)
...
a variable bias potential.\footnote{We can also
  ...
  of the unbiased distribution from Eq. \eqref{eq:Vi_target} as
...
So it is best used for the single-bin updating scheme.\footnote{Alternatively,
...
that shifts with a visit to bin $j$
...
which are sine and cosine functions.
%
The eigenvalues of the updating matrix
...
and
$\Delta z = b \delta z$
for the largest bin cutoff $b = gn/2$.
%
For $n \gg 1$,
...
\note{The table of symbols is listed in Table \ref{tab:symbols}.
  ...
  \begin{tabular}{l | p{12cm} }
    ...
    $\sigma_{ij}(t)$   &   Autocorrelation function of $\xi_i(t)$. \\
    $t, \tau$       &   Time. \\
    $T$             &   Simulation length. \\
    ...
    $\xi_{*i}(t)$ &   The noise of histogram. \\
