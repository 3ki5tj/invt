...
and we have set the index of the first eigenmode to $0$
...
\begin{equation}
  A_i =
  \frac{1}{\rho_i} \sum_{k = 1}^n \phi^*_{ki} \, \tilde{A}_k
  .
  \label{eq:gft_inv}
\end{equation}
%
...
\mathcal{F}\bigl[ h_i(t) / p_i \bigr]_k
,
$$
...
%%
%$$
%\eta_i \equiv \frac{ h_i(t) } { p_i } - \sum_{j = 1}^n \rho_j \frac{ h_j(t) }{ p_j }.
%$$
%
\begin{equation}
  \dot{\tilde v}_k(t)
  =
  \alpha(t) \, \lambda_k \, \mathcal{F} \bigl[ h_j(t)/ p_i  \bigr]_k
  .
  \label{eq:ut_diffeq_mbin}
\end{equation}


In particular, for the first eigenmode,
we have $\tilde{v}_0 \equiv v_\mathrm{av}$,
and
%
\begin{align*}
  \bav{\dot v}(t)
...
\begin{equation}
  ...
  =
  -\alpha(t) \, \lambda_k \,
  \left[{\tilde x}_k(t) - {\tilde \xi}_k(t) \right]
  .
  \notag
  %\label{eq:yt_diffeq}
\end{equation}
...
is no less than $T/(2*N)$ and no greater than $2T/N$.
%
...
$\alpha_i = 1 - \exp[-(q_{[i+1]} - q_{[i-1]}) / (t_{[i+1]} - t_{[i-1]})]$,
with
$q_{[ -1]} \equiv q_{[0]} = 0$,
