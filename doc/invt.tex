...
i.e. $p_i$ is constant for all $i$'s,
or nearly flat\cite{
dayal2004, *trebst2004, *zhang2007, barducci2008, singh2011}.
%
We will broadly call such a biased simulation
a flat-distribution-sampling (FDS) simulation.
%
To achieve this distribution, the bias potential must satisfy
%
\begin{equation}
...
continuous, rapid improvement of the bias potential,
and subsequently the PMF,
...
as a runtime average from all data collected so far,
...
\end{equation}
%
is also a white noise as $\zeta_i(t)$.
%
\note{The derivation:
$$
\begin{aligned}
...
\note{The derivative of $\dot {\bar v} = \alpha(t)$,
$$
\begin{aligned}
  \dot{ \bar v }
  &
...
\end{equation}
%
and from Eqs. \eqref{eq:error_split},
\eqref{eq:error_res1},
\eqref{eq:tk0},
and
...
the random error becomes more important than the systematic one,
because the former is roughly proportional to
the square root of the updating magnitude\cite{
  zhou2005, morozov2007, zhou2008, bussi2006}
whereas the latter is proportional to
...
would have similar precision at long times.
%
Thus, it is unnecessary to convert
a sufficiently convergent adaptive FDS simulation
to the corresponding equilibirium FDS method
...
%
In the latter, the updating magnitude is fixed,
but the bias potential over the simulation coarse is averaged
...
so that no explicit averaging is needed in the former approaches.
%
...
can be unstable upon discretization
during which the updating kernel is truncated.
%
