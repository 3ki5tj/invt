...
\newcommand{\notep}[1]{{\color{DarkPurple}\footnotesize \textsc{Note.} #1}}
...
\title{Optimal updating magnitude in adaptive flat-distribution sampling}

\notep{Deleted ``simulations'' to make the title shorter.}
...
\begin{abstract}
  We present a study on the optimization
  ...
  \notep{Deleted the ``optimal schedule'' because
  we also studied the optimization of the updating scheme,
  which is equivalent to the \emph{relative} updating magnitudes.}%
  %
  ...
  by a decreasing updating magnitude with an optimal schedule.
  \notep{Fixed the dangling gerund, ``decreasing''.}%
  %
  ...
  the bandpass updating schemes generalize
...
simulations.
\notep{The word ``often'' has been used too often in this article.}%
%
...
%
Thus,
we can overcome, at least partially, these energetic barriers in sampling
by introducing a bias potential\cite{torrie1974, *torrie1977}
...
\notep{The subject of the main clause should have been ``we'' in the previous version.}%
...
which specifies,
apart from an overall updating magnitude,
the relative change to the bias potential upon each visit.
%
...
a polynomial form,
and we will adjust the coefficients on-the-fly
to better approximate the target distribution.
\notep{Fixed the dangling infinitive, ``to approximate''.}%
%
This method serves
...
%
We present the methods in Sec. \ref{sec:methods},
...
%
Such a bias potential would satisfy
%
\notep{We haven't defined the adaptive FDS, so we can't say ``upon convergence''.}
\begin{equation}
...
one needs to gradually decrease the updating magnitude.\cite{
  belardinelli2007, *belardinelli2007jcp, *belardinelli2008, *belardinelli2016,
  zhou2005, morozov2007, morozov2009, zhou2008, min2007,
  komura2012, *caparica2012, *caparica2014,
  poulain2006, liang2007,
  crespo2010, *atchade2011, *fort2015}
\notep{Fixed the dangling infinitive, ``to reduce\dots''}
...
Next, we observe that the bias potential following Eq. \eqref{eq:wl_update}
...
\end{equation}
%
and the formal solution is % of Eq. \eqref{eq:dxdt_singlebin} is
%
\begin{equation}
  v_{*i}(T)
  =
...
such that $q(T) \to \infty$ as $T \to \infty$.
...
\begin{equation}
  ...
  \approx
  \sum_{i=1}^n \rho_i \int_{-\infty}^{\infty}
...
a visit to bin $j = i(t)$ in this case results in updates to the bias potential
...
we require the growth rate of the bias potential,
${\dot u}_i(t)$
in Eq. \eqref{eq:ut_diffeq_mbin},
to be a constant of $i$
if $h_j(t)$ were the same as $\rho_j$,
\notep{Fixed the dangling-infinitive problem.}%
i.e.,
...
in minimizing the total error. % in Sec. \ref{sec:optinitalpha}.
\notep{Fixed the dangling infinitive ``to minimize''.}
%
...
\end{align*}
%
where $\epsilon_k \equiv \tilde u_k^2 - \frac{a_0 \lambda_k \Gamma_k}{2}$.
%
Let $q_T \equiv q(T)$,
\begin{align*}
  \frac{
  ...
  \epsilon_k e^{-2 \, \lambda_k \, (a_0 \, T_0 + q_T)}
  ,
  \\
  \frac{
    \partial \Err_A(T)
...
\begin{align*}
  ...
  -\sum_{k = 1}^{n-1}
  \lambda_k \, \epsilon_k \, e^{-2\lambda_k \, (q_T + a_0 T_0)}
  =0
  .
\end{align*}
}
...
by Eqs. \eqref{eq:Lagrangian_const} and \eqref{eq:mint}, %\eqref{eq:mQ_invTa},
\begin{align}
  \alpha(t = 0)
  %=
...
and the initial bias $\epsilon_k$.\footnote{The
  ...
  although the approach often overestimates the
...
Eqs. \eqref{eq:q_opt} and \eqref{eq:mint}.\footnote{We
  ...
  for the updating magnitude, $\alpha(t)$, at any $t$.
  }
%
...
Nonetheless, we can show that it usually lies above
...
We can further construct an optimal \emph{time-dependent} updating scheme
by a superposition of single-bin updating schemes.
%
Because each eigenmode evolves independently
with an effective updating magnitude, $\lambda_k \, \alpha(t)$,
as shown in Eq. \eqref{eq:vt_diffeq_mbin},
the ideal error reduction requires
a time-dependent eigenvalue, $\lambda_k(t)$,
(hence a time-dependent updating scheme)
that allows the effective updating magnitude
to follow the eigenmode-specific optimal schedule,
$\lambda_k(t) \, \alpha(t) = 1/(t + t_{0k})$,
with $t_{0k} = \Gamma_k / \left\langle \tilde v_k^2(0) \right\rangle$,
which is borrowed from Eq. \eqref{eq:alpha_invt1} for the single-bin case.
%
In long times, however, this time-dependent optimal solution
is reduced to the single-bin updating scheme under the inverse-time schedule
as the differences among the $t_{0k}$'s become unimportant to the schedule.

\notep{This new paragraph gives a theoretically optimal updating scheme for a finite simulation.}
...
eigenmodes,
we may set the eigenvalues of the first $K$ eigenmodes to $1$
and the remaining eigenvalues to $0$:
\notep{With ``assign'', we may need to say assign $1$ to the eigenvalues.}%
%
\begin{equation}
...
the homogeneous and polynomial bandpass updating schemes.
%
...
Polynomial bandpass updating schemes}


Here we consider a class of bandpass updating schemes
constructed from a sequence of orthogonal polynomials.
%
The resulting polynomial bandpass updating schemes
simplify the bias potential as a polynomial of the collective variable.
...
with the eigenvalues given by Eq. \eqref{eq:lambda_bandpass}:
\begin{align*}
  w(z, z')
  =
...
In practice,
we often apply the above algorithm simultaneously
to an array of adjacent intervals
to cover a wide range of the collective variable, $z$.\cite{langfeld2012, pellegrini2014}
\notep{Fixed the dangling infinitive, ``to cover''.}%
%
Note that in this expanded ensemble setup of multiple intervals or multiple local umbrellas,
the updating equation for $c_0$ is no longer trivial,
but corresponds to a single-bin updating scheme
for the inter-umbrella component of the bias potential.
%
...
the inter-umbrella transitions can be handled via the frameworks of
...
and its updating equation, Eq. \eqref{eq:ck_age_update},
resembles that in the single-bin updating scheme.\cite{li2007}
...
falls under the value given by the inverse-time formula.\footnote{We
  ...
  from the WL stages to the inverse-time schedule.
...
\begin{align}
  ...
  \\
  \frac{ c_1^{(s)} }{ \sigma_c^{(s)} }
  &\approx \left. \frac{d \ln p(z) } { d z } \right|_{z = z_c^{(s)}}
  ,
  \label{eq:c1_limit}
  \\
  \frac{ \sqrt 2 \, c_2^{(s)} - 1 }{ \bigl( \sigma_c^{(s)} \bigr)^2 }
...
%
From Eqs. \eqref{eq:c0hat_limit} and \eqref{eq:c1_limit},
...
(which is equivalent to $\hat c_0^{(s)}$ here
...
\item
  ...
  in the current umbrella, $s = s(t)$, under
...
\item
  ...
  with the acceptance probability given
    ...
    Although a random mixing of the two steps
...
\item
  Update the $c_k^{(s)}$'s
  according to Eq. \eqref{eq:ck_age_update},
  and increase $t$ by $1$.
...
zhu2012, newman, *landau_binder, frenkel}
...
%
where $n^{(s)}(z)$
is the number of visits to the bin
...
%between the two tagged particles,
$p(r)$,
...
failed to reduce the errors in the $k = 7$ to $21$ modes adequately.
%
...
the intrinsic distribution is proportional to
...
\begin{figure}[h]\centering
  ...
  \caption{
    ...
    %
    (c) $c_1^{(s)}/\sigma_E$ (circles), as an approximation of
...
In conclusion,
we have studied the optimization
of the updating magnitude
...
%
The optimal updating magnitude can improve the convergence
of the bias potential,
...
The optimal schedule of the overall updating magnitude depends on the updating scheme,
...
a long preliminary run for parameter estimation,
...
We thank Dr. Y. Mei, Dr. W. Tang, J. Wang, Dr. M. W. Deem, Dr. K.-Y. Wong,
O. Nassar, Dr. C. Lai, Dr. S. Ou, D. Seckfort, %D. Stuhlsatz,
...
$\tilde{\hat u}_k(t) \approx {\tilde u}_k(t) + {\tilde f}_k(t)$,
for $k > 0$.
Thus,
\begin{align*}
  \tilde u_k(t+1) \approx \tilde u_k(t) + \lambda_k \, \alpha(t) \,
  \left[
    \tilde{\hat u}_k(t) - \tilde u_k(t)
  \right]
  ,
\end{align*}
%
...
so that the natural histogram flattening is not significantly accelerated
by the frequent non-equilibrium updates.
\notep{Fixed the dangling infinitive ``to avoid''.}
%
...
%Then,
so that
the multiplication of the matrix, $\mathbf w$,
