...
In this example, we wish to compare the single-bin,
Gaussian (Sec. \ref{sec:Gaussian_scheme}),
and homogeneous bandpass (Sec. \ref{sec:bandpass})
updating schemes in a numerical example.
...
were $2.4\times 10^{-5}$ and $3.9\times 10^{-5}$, respectively.
...
\begin{align}
  ...
  \!\!\frac{ m(\bar q) } { dm/d\bar q } \, d m
  =
  \int^{m( \bar q(t) )}_{m( q(T) )}
  \!\!\frac{ d m } { -\frac{d}{d\bar q}\ln m(\bar q) }
  %=
  %\int^{m( \bar q(t) )}_{m( q(T) )}
