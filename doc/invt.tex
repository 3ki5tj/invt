...
the adaptive Gaussian umbrella sampling below.
...
%
However, the $c_2$ term does not appear to improve histogram flatness. Ideas?
}
...
we tested the method of adaptive Gaussian umbrella sampling
...
%
We used $L = 32$ and $26$ Gaussian distributions of
the same width $\sigma_E = L$ over $[-1.7L^2, -0.9L^2]$
(the spacing between neighboring Gaussian centers and the width
were roughly the same).
%
For configuration sampling,
although one can use the Metropolis algorithm,
we found it more efficient to use
the Wolff algorithm\cite{wolff1989, newman}
at temperature $c_1/\sigma_E$
followed by a Metropolis acceptance step
to correct for the quadratic potential, $R_2$
(the acceptance ratio was no less than 92\%).
%
The results after $10^5 L^2$ steps are shown in
Fig. \ref{fig:pt_hist}.
%
...
as shown in \ref{fig:pt_hist}(a).
%
The parameter $c_1/\sigma_E$ roughly represents the local temperature,
and it demonstrated
the backbending behavior\cite{kim2006, *kim2007}
typical in a first-order phase transitions,
as shown in Fig. \ref{fig:pt_hist}(b).
%
The parameter $c_2$ represents the amount of ``pressure''
to restrain the distribution width,
and its value was maximal around the center critical region,
...
\begin{center}
  ...
  \caption{
    \label{fig:pt_hist}
    Application of the adaptive Gaussian umbrella sampling
    on the $32 \times 32$ ten state Potts model.
    %
    (a) Normalized histograms (solid lines) from adaptive Gaussian umbrella sampling
    matched the intended distributions.
    The overall histogram (dashed line) was roughly flat.
    %
    ...
    %
    (c) The parameter $c_2$ for restraining the variance
    was maximal around the critical region.
  }
\end{center}
\end{figure}
