...
the integration was performed using the trapezoidal rule\cite{press3rd}
on the grid points,
...
and used to compute the updating magnitude
...
a fixed Gaussian function centered on the current bin
...
More generally, we can replace the Gaussian function
in the above updating scheme
by a window function of different shape.
%
We call the resulting updating scheme
translationally-invariant or \emph{homogeneous}.
%see Fig. \ref{fig:mat}(b) for an example.

By morphing the window function without losing the translational invariance,
...
is the Gaussian given in Eq. \eqref{eq:rho_Gaussian}. % previously.
%
...
\item
  Set up an array of umbrellas covering the desired range,
    e.g. we may set, for $i = 1, \dots, m$,
    $z_c^{(i)} = z_{\min} + (i - 1) \, \Delta z$,
    the width, $\sigma_z^{(i)}$, to be a constant.
    For the parameters, we can set
    $c_1^{(i)}$ to the estimated mean force,
    and $c_0^{(i)} = c_2^{(i)} = 0$.
...
\item
  ...
  by Eq. \eqref{eq:transprob_st}.
  %
  The above intra-umbrella sampling steps
  and inter-umbrella transition steps
  can also be applied in random order
  with different frequencies.\cite{manousiouthakis1999}
...
After simulation,
we can use the weighted histogram analysis method
...
we will focus on the Gaussian updating scheme with width $\sigma = 0.2$ below
...
than the inverse-time schedule
...
and plotted the error components, $\langle \tilde v_k^2 \rangle$,
...
\begin{figure}[h]\centering
  ...
  \caption{
    \label{fig:lj_xerr}
    Error components, $\tilde u_k^2$'s, of the PMF
...
the optimal width was around $\sigma = 0.29$ in this case,
and it would decrease with the simulation length
and approach the limit of single-bin updating scheme,
$\sigma \to 0$, asymptotically.

The homogeneous bandpass updating scheme
yielded less error than the Gaussian updating scheme,
as shown in Table \ref{tab:lj_error}.
%
Similar to the Gaussian case,
the mode cutoff, $K$, has an optimal value,
which was around $26$ in this case.
%
The optimal cutoff would increase with the simulation length
and approach the limit of single-bin scheme, $K \to n$,
asymptotically.


The Gaussian and homogeneous bandpass updating schemes
helped reduce the error in this case because
...
the target umbrella was chosen from
any of the other umbrellas.
%
Initially, we set
...
at the inverse temperature, $\beta = c_1/\sigma_E$,
...
\begin{figure}[h]\centering
  ...
  \caption{
    ...
    %
    (b) $c_1^{(i)}/\sigma_E$, roughly corresponding to
...
and are asymptotically optimal for the non-noise modes.
%
...
%
Second, the white noise approximation
can break down for relatively short simulations.
%
...
a variable bias potential.\footnote{We can also
  ...
  from constructing an independent estimator
...
such that the evolution can be approximately
...
has a full set of orthonormal eigenvectors, $\psi_{li}$
...
$\mu_l$'s are symmetric, $\mu_l = \mu_{-l}$,
...
is defined by a Gaussian-shaped $\mu_l$.
%
