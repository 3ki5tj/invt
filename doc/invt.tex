...
%
We will show that the optimal schedule can be derived
from an equation of motion of a free particle,
and the initial updating magnitude
should be set to
half of the previous equilibrium value.
%
Particularly, the optimal schedule in the WL case
is given by
...
Further, failure to properly reduce the size of the histogram increments
...
%
Since a uniform increase of $v_i(t)$ does not affect the
resulting distribution $\pi_i(t)$ as given by Eq. \eqref{eq:h_ave},
%
the real error in $v_i(t)$ is more readily defined
...
%
\note{
The second step of Eq. \eqref{eq:hdet_linearize} follows from
$$
...
\end{equation}
%
The expression depends implicitly on the function $\alpha(t)$.
%
...
%
However, we shall assume some restrictions and approximations
...
%
Below we shall take advantage of the analogy
...
can be reduced to a convolution with the hill function,
...
and $\mu_l = 0$ for $|l| > b$.
%
We shall further impose the symmetry
%
\begin{equation}
  \mu_i = \mu_{-i}
  .
...
if the width $\sigma$ is satisified
