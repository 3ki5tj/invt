...
sampling\cite{zhou2005, zhou2008},
and for convergence
...
if the magnitude falls according to
...
comes from the interpretation where
the bias potential under the WL updating scheme,
Eq. \eqref{eq:wl_update}
as a runtime average from all data collected before time, $t$,
so that the addition of a new data point only
affects the average by a weight of $1/t$,
which is the inverse of the sample size, $t$
(cf. Appendix \ref{sec:equilerr}).
%
...
\begin{aligned}
  ...
  \right)
  \\
  &\stackrel{\mathrm{Eqs.\; \eqref{eq:h_split} and \; \eqref{eq:h_ave}}}
            {=\joinrel=\joinrel=\joinrel=\joinrel=}
  \alpha(t) \left(
...
\end{equation}
%
where $\xi(t)$ is a white noise, such that
%
$
...
from Eq. \eqref{eq:q_opt} with the truncated error definition
...
is generally weak.
%
This dependence becomes even weaker
in the single-bin updating scheme,
as it only amounts to a shift of the origin of
...
we show the errors, $\Err$,
normalized by the shifted simulation length, $T+t_0$,
so that the normalized errors
would stay constant in the single-bin updating scheme
(corresponding to the limit of $\sigma \to 0$).
%
...
correspond to local noises
that are expected to be absent from the PMF,
we can construct a
...
error\cite{zhou2005, zhou2008}
caused by the updates in adaptive FDS methods.
%
This is valid in the asymptotic regime,
where the updating magnitude is small
and the sampling is a quasi-equilibrium one\cite{barducci2008, dama2014}.
%
Then the random error is much greater than the systematic one,
because the former is proportional to the square root of the updating magnitude
whereas the latter is proportional to the magnitude itself.
%
...
with respect to the increase of sample size\cite{
  marsili2006, barducci2008}.
%
...
is to time average the bias potential\cite{
  zhou2005, zhou2008}.
%
