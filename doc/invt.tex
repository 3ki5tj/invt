...
\end{equation}
%
where we have defined the bin-average-deducted
histogram, $\eta_{*i}(t)$, in a similar way to
$v_{*i}(t)$ [cf. Eq. \eqref{eq:x_def}].


We now split the histogram into
a deterministic averaged part, $\langle \eta_i(t) \rangle$,
...
as white noise,
and assume its validity for long simulations.
%
The bin-average-deducted fluctuation, $\zeta_{*i}(t)$,
%%
%\begin{equation}
%  \zeta_{*i}(t)
%  \equiv
...
%\end{equation}
%%
would also be governed by white noise as is $\zeta_i(t)$, i.e.,
\begin{equation}
...
\begin{equation}
  \dot v_{*i}(t)
  =
  \alpha(t) \, \left[ \langle \eta_{*i}(t) \rangle + \zeta_{*i}(t) \right]
  \approx
...
and for the bin average of any quantity, $f$, we have
...
a bin-average-deducted quantity always
has a vanishing first mode.
...
\end{equation}
%
Deducting the bin average yields
$$
  \dot {\tilde v}_{*k}(t)
  = \alpha(t) \, \lambda_k \, {\tilde \eta}_{*k}(t)
  \approx
  -\alpha(t) \, \lambda_k \,
  \bigl[ {\tilde v}_{*k}(t) - {\tilde \zeta}_{*k} (t) \bigr]
  ,
$$
...
%
By Eq. \eqref{eq:ftstar},
we can start the last sum %in Eq. \eqref{eq:xt2_sum}
from $k = 1$,
%
\begin{equation}
  \Err(T)
...
$\kappa_k(t) \equiv \langle {\tilde \zeta}_{*k}(0)
\, {\tilde \zeta}_{*k}(t) \rangle$,
as:
%
\begin{equation}
  \Gamma_k
  ...
  = \sum_{t = -\infty}^\infty
  \left\langle {\tilde \zeta}_{*k}(0)
            \, {\tilde \zeta}_{*k}(t) \right\rangle
  .
  \label{eq:Gamma_sum}
\end{equation}
%
...
%\end{equation}
%
We may understand Eq. \eqref{eq:Gamma_sum}
as an attempt of modeling
a general histogram fluctuation, ${\tilde \zeta}_{*k}(t)$,
...
\begin{equation}
  ...
  + \tilde u_k^2 \, e^{-2\, \lambda_k \, T_0}
  ,
  \label{eq:xt2_eql1}
\end{equation}
%
and if no eigenvalue is zero,
then we can ignore the second term
...
$\sum_{i=1}^n \rho_i \langle v_{*i}(t) \, v_{*i}(0) \rangle
=\sum_{k=1}^{n-1} \langle {\tilde v}_{*k}(t) \, {\tilde v}_{*k}(0) \rangle$,
so
\begin{align*}
\sum_{i=1}^n \rho_i G_i
&=\sum_{t=-\infty}^\infty\sum_{i=1}^n
  \rho_i \langle v_{*i}(t) \, v_{*i}(0) \rangle \\
  &=\sum_{t=-\infty}^\infty\sum_{k=1}^{n-1}
  \langle {\tilde v}_{*k}(t) \, {\tilde v}_{*k}(0) \rangle
  =\sum_{k=1}^{n-1} \Gamma_k.
\end{align*}
}
%
...
we first note that
the difference, $u_{*i} = u_i - \bav{u}$,
differs from the actual deviation, $v_{*i}$,
...
${\tilde V}_{*k}$ and ${\tilde v}_{*k}$, would also share the same variance.
%
But since the average of ${\tilde v}_{*k}$ is zero,
we have
\begin{equation*}
