...
to an average in an ensemble consists of
...
of the bin-average deduction is to
...
[the subscript ``$_*$'' is dropped by Eq. \eqref{eq:ftstar}].
...
we discuss several ways of estimating $\Gamma_k$'s.
%
...
\footnote{We solved the equation for $\ln q(T)$ using
    a variant of the Newton-Raphson method
    with the bisection method as fallback.
    The initial value set to $q(T) = \ln(1+T\,a_0/2)$.
    Other general methods can be more effective\cite{press3rd}.}
%
...
numerically integrating Eqs. \eqref{eq:q_opt} and \eqref{eq:mint}.\footnote{
  We have used the trapezoidal rule\cite{press3rd}
  on an adaptively defined grid on $[0, q(T)]$ of $N+1$ points
  (we chose $N=20000$)
  such that the resulting change of $\Delta t_{[i]} \equiv t_{[i+1]}-t_{[i]}$
  is no less than $T/(2N)$ and no greater than $2T/N$.
  %
  This requires a few (usually $2$ or $3$) rounds of iterations
  to determine the constant $C$.
  %
  The first guess of $C$ is obtained from integrating
  Eq. \eqref{eq:mint} on a uniform grid.
  %
  Then in each of the next few rounds, we determine the grid points
  as follows.  For grid point $i = 0, 1, \dots, N - 1$,
  we determine the stride
  $\Delta q_{[i]} \equiv q_{[i+1]} - q_{[i]}$
  by a few iterations.
  Starting with the guess, $\bigl(q(T)-q_{[i]}\bigr)/(N-i)$,
  if the resulting increment $\Delta t_{[i]}$
  for grid point $i$ is less than $T/(2N)$,
  we multiply $\Delta q_{[i]}$ by $3/2$
  unless the resulting $q_{[i+1]} > q(T)$,
  or if $\Delta t_{[i]} > 2T/N$,
  we divide $\Delta q_{[i]}$ by $2/3$.
  %
  Once the grid points are defined,
  we will determine the new value of $C$,
  and if the new value differs from the old estimate
  by more than $10\%$, we start another round of iteration.
%
%The curve $q(t)$ is then defined on the grid points,
%$(q_{[i]}, t_{[i]})$,
%
The updating magnitude can then be obtained from numerical differentiation:
...
The above procedure is still somewhat idealized.
%
The preliminary run in step \ref{step:prerun}
needs to be sufficiently long to precisely
estimate the $\Gamma_k$'s and the initial bias, $\epsilon_k$
(for which one may use the histogram-corrected bias potential).
%
However,
