...
allows us to predict the error analytically
[cf. Appendix \ref{sec:invt_schedule}],
and it serves as a model of verifying our theory.
...
While the optimal schedules of the Gaussian updating schemes
are rather complex,
those of the bandpass updating schemes
...
%
However, the ruggedness disappears
...
\begin{center}
  ...
  \caption{
    \label{fig:sinc}
    (a) Optimal schedules of the bandpass
    updating schemes for a periodic variable
    with $K = 4$,
    and for a non-periodic variable
    with $K = 8$.
    (b) Corresponding updating kernels
    compared to a Gaussian with a similar width,
    $\sigma = 5$.
    The lines are a guide to the eyes.
    Inset. Updating matrix according to Eq. \eqref{eq:w_band_refl}.
  }
\end{center}
\end{figure}
...
that the lowest error was achieved
by the widest window for the shorter simulation
of $T = 10^8$,
while the minimum moved to $\sigma = 0$
as the simulation lengthens to $T = 10^{10}$.
%
However,
...
In all cases,
the bandpass scheme
appeared to be a superior
alternative to the Gaussian updating scheme.
...
\begin{center}
  ...
  \caption{
    ...
    for the Gaussian
    and the brick-wall bandpass updating schemes.
    %
    In the latter case,
...
brick-wall bandpass updating schemes that are
