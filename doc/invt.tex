...
\begin{abstract}
  ...
  belongs to a class of updating schemes
...
\end{equation}
%
These numbers constitute a \emph{updating kernel}.
...
%
For example,
if we know in advance that the intrinsic
PMF is sufficiently smooth such that
the error modes can be limited to $k \le K$.
%
Then we can manually construct
an updating scheme with
$$
...
the updating kernel can be inversely
found from Eq. \eqref{eq:mu_from_lambda_pbc}
or \eqref{eq:mu_from_lambda_refl}.
%
Then,
...
\frac{1}{T^2}
\sum_{ t, t' = 1 }^T
\langle \eta_k(t) \, \eta_k(t') \rangle
\\
&\approx
\frac{1}{T}
\sum_{ \tau = -\infty }^{ \infty }
\langle \eta_k(t) \, \eta_k(t + \tau) \rangle
\\
&=
\frac{1}{T}
\int_{-\infty}^\infty
...
verifying Eq. \eqref{eq:alpha_invt}.
%
