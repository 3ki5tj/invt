...
\footnote{We applied the Newton-Raphson method on $\ln q(T)$
    with the initial value set to $q(T) = \ln(1+T\,a_0/2)$.
    Other root-finding methods can be more effective\cite{press3rd}.}
%
...
which will be used to compute the optimal $q(T)$
and schedule later.
