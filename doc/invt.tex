...
\note{This follows from
  ...
  \begin{aligned}
    ...
    \\
    p_i
    &\propto p^*_i \, e^{-v^{0, \mathrm{corr}}_i}.
  \end{aligned}
  $$
}
...
dama2014}.
%
We define a bin-average-deducted bias potential as
%
\begin{equation}
...
%
From Eqs. \eqref{eq:x_def} and \eqref{eq:vav_def},
we have
%
\begin{equation}
...
or zero otherwise,
and we have defined a relative histogram
$\eta_i(t) \equiv h_i(t) /p_i$
in the last step.
%
...
We now try to split the histogram into a deterministic averaged part
and a random fluctuation part, $\zeta_i(t)$,
%
\begin{equation}
  \eta_i(t) = \langle \eta_i(t) \rangle + \zeta_i(t).
  \label{eq:h_split}
\end{equation}
%
...
%
Deducting the bin average [similar to Eq. \eqref{eq:vav_def}],
we have
%
\begin{align}
  \langle \eta_i(t)  - \eta_\mathrm{av}(t) \rangle
  \equiv
  \langle \eta_i(t) \rangle -
  \sum_{j=1}^n \rho_j \langle \eta_j(t) \rangle
  &\approx
  - x_i(t)
  .
  \label{eq:sh_ave}
...
%
and we will approximate it as white noise.
%i.e.
...
%
The bin-average-deducted fluctuation
%
\begin{equation}
  \xi_i(t)
  \equiv
  \zeta_i(t) - \zeta_\mathrm{av}(t)
  \equiv
  \zeta_i(t)
  -
  \sum_{ j = 1 }^n
...
\end{equation}
%
would also be governed by white noise as is $\zeta_i(t)$:
%i.e. we may write
\begin{equation}
...
$\sum_{t = -\infty}^\infty \langle \xi_i(t) \, \xi_i(0) \rangle$.
%
\note{More explicitly,
...
then
$
  p_i p_j \langle \zeta_i(t) \, \zeta_j(t') \rangle
  =
...
From Eqs.
\eqref{eq:x_def}, \eqref{eq:vt_diffeq}-\eqref{eq:xi_def},
%\eqref{eq:v_def},
%\eqref{eq:x_def},
%\eqref{eq:vav_def},
%\eqref{eq:rhox_sum},
...
\begin{equation}
  \dot x_i(t)
  =
  \alpha(t) \, \left[ \eta_i(t) - \eta_\mathrm{av} \right]
  \approx
...
%%&=
%%\int_0^t \int_0^t \dot y(t') \, \dot y(t'')
...
%&=
%\frac{d}{dt} \int_0^T
%  \dot y\bigl( q(t') \bigr) \, \kappa(t' - t) \, dt'
%\\
%&= \int_0^T
%  \ddot y\bigl( q(t') \bigr) \, \kappa(t' - t) \, dt',
%\end{aligned}
%$$
...
and the other constant, $c$, has been determined
from the $t = T$ case
in the second step.
%
...
\end{equation}
%
and the bin average, Eq. \eqref{eq:vav_def} can be written as
\begin{equation}
  v_\mathrm{av} = \mathcal F[v_i]_0 = \tilde v_0
  .
  \label{eq:vav_vt0}
\end{equation}

We can now diagonalize
...
\begin{equation}
  \dot{\tilde v}_k(t)
  =
  \alpha(t) \, \lambda_k \, {\tilde \eta}_k(t)
  .
...
\begin{aligned}
  ...
  \left[
    {\tilde \eta}_k(t)
    -
    {\tilde \eta}_\mathrm{av}(t) \, \delta_{k0}
  \right]
  \\
  &\approx
  -\alpha(t) \, \lambda_k \,
  \bigl[ {\tilde x}_k(t) - {\tilde \xi}_k (t) \bigr]
  ,
\end{aligned}
$$
where we have used the linear approximation
introduced in Sec. \ref{sec:sbin_diffeq}.
...
%
From Eq. \eqref{eq:eigenmode0},
the first eigenmode,
${\tilde x}_0 = \sum_{j=1}^n \rho_j \, x_j$,
vanishes identically by Eq. \eqref{eq:rhox_sum}.
%
Thus, we can start the last sum %in Eq. \eqref{eq:xt2_sum}
from $k = 1$,
%
\begin{equation}
  \Err(T)
...
$\kappa_k(t) \equiv \langle {\tilde \xi}_k(0) \, {\tilde \xi}_k(t) \rangle$,
as:
%
\begin{equation}
  \Gamma_k
  ...
  = \sum_{t = -\infty}^\infty
  \left\langle {\tilde \xi}_k(0) \, {\tilde \xi}_k(t) \right\rangle
  .
  \label{eq:Gamma_sum}
\end{equation}
%
...
essentially maps the histogram fluctuation, ${\tilde \xi}_k(t)$,
...
%
This serves
as a generalization of Eq. \eqref{eq:dydt_const}.
%
...
\item
Compute $\Gamma_k$'s from Eq. \eqref{eq:varXt}
    (cf. Appendix \ref{sec:Gamma_measure}).

\item
Compute the optimal $q(T)$ by solving Eq. \eqref{eq:opt_qT}
    using e.g. the Newton-Raphson or bisection method\cite{press3rd}.
%
...
%($i = 0, 1, \dots, N$)
We have used the standard trapezoidal rule\cite{press3rd},
on $N+1$ grid points on $[0, q(T)]$ that are defined adaptively
such that the resulting change of $t_{[i+1]}-t_{[i]}$
is no less than $T/(2N)$ and no greater than $2T/N$.
%
...
and the updating magnitude can be obtained from numerical differentiation:
    $\alpha_i = [1 - e^{-\lambda_1 (q_{[i+1]} - q_{[i-1]}) / (t_{[i+1]} - t_{[i-1]})}]/\lambda_1$,
with
$q_{[ -1]} \equiv q_{[0]} = 0$,
...
similar to those leading to Eq. \eqref{eq:xt2_sum}.
%
%
%
\note{The derivation.
...
=\sum_{k=1}^{n-1} \langle {\tilde x}_k(t) \, {\tilde x}_k(0) \rangle$,
so
\begin{align*}
\sum_{i=1}^n \rho_i G_i
&=\sum_{t=-\infty}^\infty\sum_{i=1}^n \rho_i \langle x_i(t) \, x_i(0) \rangle \\
  &=\sum_{t=-\infty}^\infty\sum_{k=1}^{n-1} \langle {\tilde x}_k(t) \, {\tilde x}_k(0) \rangle
  =\sum_{k=1}^{n-1} \Gamma_k.
\end{align*}
}
%
...
%
Thus,
%Since $H_i(t)$ is the average of the instantaneous histogram, $h_i(t)$,
%we may apply Eq. \eqref{eq:av_recur}, and
%
\begin{align}
  u_i(t+1)
  &=
...
we can use Eq. \eqref{eq:xt2_eql} to
...
of the bias potential, $u_i$.
%
To circumvent the problem, we
modify Eq. \eqref{eq:xt2_eql} as
%
\begin{equation}
  \operatorname{var} {\tilde X}_k
  =
  \frac{1}{2} \,
...
\bav{u}
\equiv
\sum_{ j = 1 }^n \rho_j \, u_j.
$

To show Eq. \eqref{eq:varXt},
we first note that
the difference, $u_i - \bav{u}$,
...
${\tilde X}_k$ and ${\tilde x}_k$, would also share the same variance.
%
But since the average of ${\tilde x}_k$ is zero,
we have
$$
\operatorname{var} {\tilde X}_k
=
\operatorname{var} {\tilde x}_k
=
\left\langle {\tilde x}_k^2 \right\rangle.
$$
Finally, by using Eq. \eqref{eq:xt2_eql},
we reach Eq. \eqref{eq:varXt}.

\note{In practice, Eq. \eqref{eq:varXt}
...
%$\left\langle {\tilde x}_k^2(0) \right\rangle$
...
\note{The table of symbols is listed in Table \ref{tab:symbols}.
  ...
  \begin{tabular}{l | p{12cm} }
    ...
    ${\tilde x}_k(t)$        &   Mode of the shifted bias potential. \\
    ${\tilde X}_k$           &   Mode of the original bias potential. \\
