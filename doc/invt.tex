...
in each step, the system hops to the previous $i - 1$ bin
or the next bin $i + 1$ with equal probability $1/2$,
ignoring out-of-boundary transitions.
%
We shall further assume that
the eigenvectors of the $\mathbf w$ matrix
coincide with the those of the transition matrix
of the above sampling process
(cf. Sec. \ref{sec:band-matrix}).
%
Since the transition matrix is tridiagonal,
as given by Eq. \eqref{eq:wtridiag} with $\mu_1 = 1/2$,
the eigenvalues are given
by Eq. \eqref{eq:wband_eigenvalue} as
$$
\Lambda_i = 1 - 2 \sin^2 \frac{ (i - 1) \, \pi } { 2 \, n }.
$$
Note, however, these $\Lambda_i$'s are the eigenvalues of
the transition matrix of sampling scheme,
not to be confused with the eigenvalues, $\lambda_i$'s,
of the $\mathbf w$ matrix.
%
The correlation function $\kappa_i(t)$ of
...
=
\cot^2 \left( \frac{ i \, \pi } { 2 \, n } \right).
