...
\item
  ...
  with the acceptance probability given
    by Eq. \eqref{eq:transprob_st}.\cite{*[{Although
    the intra-umbrella sampling step
    and inter-umbrella transition step
    are executed in sequential instead of random order,
    the sampling scheme is still valid,
    as shown in }] [{}] manousiouthakis1999}
...
and plotted the error components,
...
for two simulations with equal lengths and initial updating magnitudes,
the simulation under the Gaussian scheme with the optimal schedule
has a larger updating magnitude than
the value given by the inverse-time formula,
which is optimal for the single-bin scheme.
%
From Eqs. \eqref{eq:mass_func} and \eqref{eq:mass_distr},
we have
\begin{align}
  -\frac{ d \ln m(\bar q) }{ d \bar q }
  %=
  %-\frac{ dM^2(\bar q)/d\bar q } { 2 M^2(\bar q) }
  =
...
%
Let us use Eqs. \eqref{eq:intmQ_tT} and \eqref{eq:dlnmdq}
to compare the time of
reaching a certain updating magnitude, $\alpha_1$,
in the two simulations:
%
\begin{align}
  \frac{t}{T}
  %=
  %\int_{\bar q(t)}^{q(T)} m(\bar q) \, d\bar q
  =
  \int_{m( \bar q(t) )}^{m( q(T) )}
  \!\!\frac{ m(\bar q) } { dm(\bar q)/d\bar q } \, d m
  =
  \int^{m( \bar q(t) )}_{m( q(T) )}
  \!\!\frac{ d m } { \bar \lambda(\bar q) }
  ,
  \label{eq:t_int}
\end{align}
