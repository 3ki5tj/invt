...
Eqs. \eqref{eq:q_opt} and \eqref{eq:mint}.\footnote{We
  ...
  where $i = 0, \dots, \mathcal N$
  with $\mathcal N = 20000$.
  %
...
\item
  Set up an array of Gaussian umbrellas covering the desired range,
    ...
    $c_1^{(i)}/\sigma_z^{(i)}$ to the estimated mean force,
    and $c_0^{(i)} = c_2^{(i)} = 0$.
...
Early in simulation, however,
we still have to resort to the WL stage-by-stage
technique even for the bandpss updating schemes.
%the random error,
...
but modify it to avoid the problem of not
