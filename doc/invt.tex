...
Compute the optimal schedule by
  ...
  We have used the trapezoidal rule
  on a $N+1$ point logarithm grid over $[0, q(T)]$:
    $q_{[i]} = \bigl(q(T) + 1\bigr) - \bigl(q(T)+1\bigr)^{1-i/N}$,
  where $i = 0, \dots, N$, and we chose $N=20000$.
%
The integration yields a curve $q(t)$ on the grid points,
$(q_{[i]}, t_{[i]})$.
%
