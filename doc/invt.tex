...
(cf. Sec. \ref{sec:FDS}).
%
To circumvent the problem, we shall
...
\begin{tikzpicture}
  ...
  \node[draw, text width={width("System and")}]
  ...
  \node[draw, text width={width("Schedule ")}]
    (alpha) at (5.2, 1) {Optimal schedule, \\ $\alpha(t)$};

  \node[draw, text width={width("Error ")}]
    (err) at (7, 1) {Error, \\ $\Err$};

  \node[inner sep=0, minimum size=0] (M1) at (3.7, 1) {};
  \node[inner sep=0, minimum size=0] (M2) at (4.0, 1) {};
  \node[inner sep=0, minimum size=0] (N2) at (4.0, -0.2) {};
  \node[inner sep=0, minimum size=0] (N3) at (6.2, -0.2) {};
  \node[inner sep=0, minimum size=0] (M3) at (6.2, 1) {};
...
and thus the relation can be readily inverted.
%
...
to take into account the initial error.
