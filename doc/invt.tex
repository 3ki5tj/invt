...
to an average in an ensemble consists of many similar simulation copies
...
\end{equation}
%
which means that the only the only effect
of bin average deduction is to
...
$\kappa_k(t) \equiv \bigl\langle {\tilde \zeta}_{*k}(0)
\, {\tilde \zeta}_{*k}(t) \bigr\rangle$,
as:
%
\begin{equation}
  \Gamma_k
...
\begin{align}
  a_0 \, M\bigl( q(T) \bigr)
  +
  \frac{ 2 }{M\bigl( q(T) \bigr)}
  \sum_{k = 1}^{n-1}
  \lambda_k \, \epsilon_k \, e^{-2\lambda_k \, q(T)}
  \notag \\
  = \frac{2}{T} \int_0^{q(T)} M(Q') \, dQ'
  .
\label{eq:opt_qT}
\end{align}
%
...
\footnote{Our implementation used a variant of the Newton-Raphson method
    with bracketing, and the initial value is set to $q(T) = \ln(1+T\,a_0/2)$,
    but other methods might be more effective\cite{press3rd}.}
%
