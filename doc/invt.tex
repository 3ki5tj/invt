...
The challenge of sampling a multimodal distribution, $p_i$,
with deep valley(s) can often be eased by
biased sampling targeting a smoother distribution, $\rho_i$.
%
%
...
\begin{align}
  ...
  \label{eq:ET_average}
\end{align}
where $\Gamma \equiv \sum_{i=1}^n \rho_i \, G_i$.
%\begin{equation}
...
%
$\Err(T) = [ \Err(0) \, t_0^2 + \Gamma \, T ]/(t_0 + T)^2$,
%
which is minimal at
%
\begin{equation}
  t_0 = \frac{ \Gamma } { \Err(0) }
  ,
  \notag
...
the bin average of any quantity, $x_i$,
...
[but the subscript ``$_*$'' is dropped by Eq. \eqref{eq:xtstar}].
...
at large $\bar q$ (cf. Appendix \ref{sec:seplambda}),
...
it recovers $t_0 = \Gamma/\Err(0)$ %Eq. \eqref{eq:t0_singlebin}
under the assumption of Eq. \eqref{eq:xt2_eql}.
%
From Eq. \eqref{eq:xt2_eql}, we have
$$\Err(0) = \frac{1}{2} a_0 \sum_{k=1}^{n-1} \Gamma_k
= \frac{1}{2} \, a_0 \, \Gamma.$$
}
%
...
would be inaccurate because the resulting deviation in the bias potential
was no longer small,
...
\end{equation}
%
Below we shall use capital letter to denote
time average: for variable $x$, we define
\begin{equation}
...
$$
G_{<K}(T) \equiv \sum_{k=1}^{K-1} \tilde F_k^2(T),
$$
...
\section{\label{sec:seplambda}
Updating scheme
with a set of well-separated eigenvalues}


We will study updating schemes
with a set of well-separated eigenvalues,
as a model for the large $k$ eigenmodes
of the Gaussian updating scheme.
%
We will show that the mass distribution will have
an inverse-$\bar q$ profile,
and derive a upper bound for the asymptotic error.
...
$$
  ...
  \le
  \sum_{k = 2}^n
  \sqrt{ \Gamma_k } \, \lambda_k \, e^{ -\lambda_k \, \bar q }
  .
$$
Thus,
...
%
$$
\frac{T}{(T+t_0)^2} \, \sum_{k=1}^{n-1} \Gamma_k
\le
\frac{1}{T} \sum_{k=1}^{n-1} \Gamma_k
\le
\frac{1}{T} \left( \sum_{k=1}^{n-1} \sqrt{ \Gamma_k } \right)^2
.
$$
...
%
First, we have for any mode $k^*$ $(k^* > 0)$,
%
\begin{align}
  M(\bar q)
  &
  ...
  e^{ -\lambda_{ k^* } \, \bar q }
  \ge
  \frac{ \sqrt{ \Gamma_{ k^* } } } { \bar q }
  ,
\label{eq:MQ_lbound1}
\end{align}
%
where the first inequality
would approach an equality if the $k^*$ mode
dominates the mass function, $M(\bar q)$,
and the second inequality
represent the limit of $\lambda_{k^*} \to 1/\bar q$.
%
...
\begin{align}
  ...
  \, \Gamma_k \, \lambda_k^2
  \int_0^{q(T)}
  \frac{ e^{-2 \, \lambda_k \, \bar q } } { M(\bar q) }
  \,  \, d \bar q
  \notag
  \\
  &\le
...
From Eq. \eqref{eq:Gamma_local},
we know that $\Gamma_k$ would quickly approach a constant.
%
Then Eq. \eqref{eq:y2_ubound} shows that
the asymptotic error profile would also
approach a plateau in this case
(cf. Fig. \ref{fig:lj_xerr}
for a Gaussian example).
%
However, to reach this upper bound
...
Finally, from Eqs. \eqref{eq:MQ_lbound1} and \eqref{eq:Gamma_local}
we know that for a large $\bar q$, which implies a sufficiently
large matching mode $k^*$ with $\lambda_{k^*} \approx 1/\bar q$,
the mass function would be roughly proportional to $1/\bar q$,
and so is the mass distribution, i.e.
\begin{equation}
  m(\bar q) \propto \frac{ 1} { \bar q },
  \qquad
  \mbox{for large $\bar q$.}
  \label{eq:m_invq}
\end{equation}
