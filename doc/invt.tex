...
with the normalized histogram, $H_i$, from the simulation.
%
...
simulation period to accumulate a precise histogram,
and thus disallows
...
can often sample the desired distribution, $p_i$,
even with a constant magnitude,
$\alpha(t) = \alpha$,
...
komura2012, *caparica2012, *caparica2014}.
%
An interesting explanation of this result\cite{
...
Note that the error weighting distribution, $\pmb\rho$,
defined above
is independent of
the target distribution,
$\mathbf p = (p_1, \dots, p_n)$.
%
In practice, $\pmb\rho$ is usually flat,
i.e. $\rho_i \equiv 1/n$,
even when $\mathbf p$ is not
...
%
and we will assume it is white, i.e.
...
i.e. we may write
$$
  \langle \xi_i(t) \, \xi_i(t') \rangle = G_i \, \delta(t - t'),
$$
for some $G_i$.
%
\note{The derivation:
...
We recall that
the formal solution of Eq. \eqref{eq:dxdt_WL} is
%
\begin{equation}
x_i(T)
=
...
following the $t = 0$ case of Eq. \eqref{eq:expqt}\big].
%
...
By using the optimal schedule in Eq. \eqref{eq:ET_average},
...
is readily satisfied in practice:
...
With Eq. \eqref{eq:w_detailedbalance},
the scaled matrix, $\sqrt{ \rho_i/\rho_j } \, w_{ij}$,
is symmetric.
Thus, we can diagonalize $\mathbf w$ with a set of
...
we can again approximate $\theta_k(t)$
as white noise:
...
\end{equation}
%
Then, the asymptotic error is simplified as
%
\begin{align}
  \Err_A(T)
  &=
...
the optimized asymptotic error is given by
%
\begin{equation}
  \Err_A(T)
  =
...
\subsection{\label{sec:procedure}
...
%
In step \ref{item:lambda},
one can use Eq. \eqref{eq:wband_eigenvalue}
for a homogeneous updating scheme.
For a discrete variable,
the updating scheme should be stabilize
using the procedure in Appendix \ref{sec:stabilize_wband}
to ensure all eigenvalues are nonnegative.
%
In step \ref{item:Gamma},
the details of measuring $\Gamma_k$'s
can be found in Appendix \ref{sec:Gamma_measure}.

In step \ref{item:qT},
since $m(Q)$ is a decreasing function of $Q$,
one can solve Eq. \eqref{eq:opt_qT}
for a sufficiently small $a_0$
by the bisection method.
%
Starting from the interval $[Q_L, Q_R]$,
where $m(Q_L) > 2/(T\,a_0) > m(Q_R)$,
one can iteratively refine the interval
by substituting $Q_M = (Q_L + Q_R)/2$
for either $Q_L$ and $Q_R$,
depending on the sign of $m(Q_M) - 2/(T \, a_0)$.

In step \ref{item:qopt},
the integrals in Eqs. \eqref{eq:q_opt} and \eqref{eq:mint}
are done on an even grid of $Q \in [0, q(T)]$
of $N$ points
using the standard trapezoidal rule.
%
The curve $q(t)$ is then defined on the grid points,
$(Q_i, t_i)$ ($i = 0, \dots, N$),
which can be numerically differentiated
to get the corresponding values of the updating magnitude:
$\alpha_i = (Q_{i+1} - Q_{i-1}) / (t_{i+1} - t_{i-1})$
(for the boundary values, we define
$Q_{-1} = t_{-1} = 0, Q_{N+1} = q(T), t_{N+1} = T$).
This results in a table of $(t_i, \alpha_i)$
that can be used for linear interpolation
to get the updating magnitude at any given time, $t$.
...
then from Eq. \eqref{eq:ET_average}, we have
$q(t) = \alpha_s \, t$,
where $t$ denotes the number of steps in stage $s$.
%
Then
%
\begin{align}
...
we map the overall simulation time, $t^{(s)}$,
...
as the initial updating magnitude, $\alpha(t^{(s)} = 0)$,
...
Here, we give some detailed derivations
...
and the eigenvalues are given by
  Eq. \eqref{eq:wband_eigenvalue_refl}.
...
We give two examples of the homogeneous updating schemes.
