...
\begin{equation}
  ...
  \approx
  \alpha(t) \,
  \sum_{j=1}^n w_{ij} \, \frac{ h_j(t) } { p_j }
  .
...
is now mode-dependent,
and the minimization of the final error
...
\begin{equation}
  ...
  \sqrt{
    ...
    \Gamma_k \, \lambda_k^2 \, e^{-2 \, \lambda_k \, q'}
  }
  .
  \notag
  %\label{eq:mass_func}
...
\begin{center}
  \begin{tikzpicture}[very thick, scale=2.5, every node/.style={scale=1.5}]
    ...
    \draw[->] (0, 0) -- node[below, inner sep=7mm] {$\bar q$} (2.8, 0);
    \draw[->] (0, 0) -- node[above, rotate=90] {$m(\bar q)$} (0, 1.9);
  ...
  \caption{
    ...
    $\alpha(t)$,
    via the normalized mass distribution, $m(\bar q)$.
    %
    \note{
...
\begin{table}[h]\footnotesize
  \caption{\label{tab:m_and_a}
    Variable transformation between the
  ...
  \renewcommand\arraystretch{2.3} % spacing between rows
  \begin{tabular} { c c c }
    \hline
    $m^*(\bar q^*) = -\dfrac{d t^*}{d\bar q^*}$ &
    $\alpha^*(t^*) = -\dfrac{d\bar q^*}{d t^*}$ &
    $\bar q^*(t^*)$
    \\
    \hline
    $\dfrac{c \, a}{\left( {\bar q}^* + q^*_0 \right)^{a+1}}$ &
    $\dfrac{c^{1/a} \, (1/a)}{\left( t^* + t^*_0 \right)^{(1/a)+1}}$ &
    $\left( \dfrac{c}{t^* + t^*_0} \right)^{1/a}$
    %$\,\begin{aligned}
    %  &a, q^*_0 > 0, \\
    ...
    %\end{aligned}$
    \\
    $\dfrac{1}{\gamma \, {\bar q}^* + \frac{\gamma}{e^\gamma - 1}}$ &
    $\dfrac{\gamma}{1-e^{-\gamma}} \, e^{-\gamma \, t^*}$ &
    $\dfrac{e^{-\gamma \, t^*} - e^{-\gamma}}{1-e^{-\gamma}}$
    \\
    $\dfrac{\gamma}{1 - e^{-\gamma}} e^{-\gamma \, {\bar q}^*}$ &
    $\dfrac{1}{ \gamma \, t^* + \frac{\gamma}{e^\gamma - 1} }$ &
    $\dfrac{\ln\left[(1-e^{-\gamma}) \, t^* + e^{-\gamma} \right]}{-\gamma}$
    \\
    \hline
    \multicolumn{3}{c}{
    Here,
...
%the shape of the mass distribution, $m(\bar q)$,
%also determines how much the optimal schedule
%would depend on the simulation length, $T$.
%%
%Since both $M(\bar q)$ and $m(\bar q)$ decrease monotonically with $\bar q$,
%Eq. \eqref{eq:opt_qT} shows that
%the optimal $q(T)$ increases with
%the simulation length, $T$,
%more so if $M(\bar q)$ has a long tail.
%%
%Thus, if $M(\bar q)$ decays slowly because of
...
%
%If $M(\bar q)$ decays rapidly,
%the distribution $m(\bar q)$ does not critically
...
%%
%If, however, $M(\bar q)$ has a long tail
...
In particular, for the Gaussian updating scheme,
...
\begin{align}
  ...
  &=
  \sum_{t=-\infty}^\infty \sum_{k=1}^{n-1} \langle {\tilde v}_k(t) \, {\tilde v}_k(0) \rangle
  \notag
  \\
  &=
  \sum_{t=-\infty}^\infty \sum_{i=1}^n \rho_i \langle v_{*i}(t) \, v_{*i}(0) \rangle
...
the single-bin updating scheme
supplied with a perfect filter
for the noise modes,
and zero eigenvalues are assigned to the noise modes.
%
For example,
by assuming
...
\end{equation}
%
as a generalization of Eq. \eqref{eq:Emin_singlebin}
for the single-bin updating scheme.

Since the bandpass updating scheme is
...
be limited to first few modes, $k = 1, \dots, K$,
as the bandpass updating scheme
only partially flattens the histogram
of these modes (cf. Appendix \ref{sec:hfluc}).
...
%
The Gaussian scheme produced less error than
the single-bin scheme in the $\varrho^* = 0.1$ case,
but not in the $\varrho^* = 0.8$ case.
%
In the low density case,
when the local noise contributes significantly to the total error,
the Gaussian scheme
yielded less error than the single bin scheme,
and the optimal schedule was more effective
in reducing the error than the inverse-time schedule.
%
However, in the high-density case
the error was dominated by the $k=1$ eigenmode
corresponding to the longest range motion,
as $\Gamma_1 \approx \Gamma$,
and the reduction of local noise
had little effect on the total error.
%
The bandpass updating scheme behaved similarly,
and in the low density case when
local noise is important,
this scheme (with the mode cutoff $K = 20$)
was superior to the Gaussian counterpart.

The Gaussian updating scheme was most effective
at an optimal width, which
was $\sigma = 0.37$ for $\varrho^* = 0.1$
or $0.07$ for $\varrho^* = 0.8$ in this case,
although it depended on the simulation length.
%
%
To further investigate its behavior
we show in Fig. \ref{fig:lj_xerr}
the error components, $\langle \tilde v_k^2 \rangle$,
against the eigenmode index
...
it also slowed down the error reduction of these modes
[cf. Eq. \eqref{eq:vt_diffeq_mbin} and \eqref{eq:lambda_Gaussian}].
%
...
%
Even without the optimal schedule,
it was also possible to roughly
recover the single-bin result by using the correction formula
based on the histogram, Eq. \eqref{eq:uhatav}.
%
This correction usually improves
the estimate for the small-$k$ modes
...
\end{equation}
%
We define a time average for variable, $f$ as
\begin{equation}
\overline{f}(t) \equiv \frac{1}{t} \sum_{\tau=1}^t f(\tau)
.
  \label{eq:timeav}
\end{equation}
%
Then after time averaging, Eq. \eqref{eq:uhatt} becomes
\begin{align}
  \overline{ \hat u_i }(t)
  &\approx
  \overline{ u_i } (t)
...
\end{align}
%
where we have introduced average histogram
%
\begin{align}
  H_i(t)
...
$$
\overline{\hat u_i}(t) \approx \overline{ u_i }(t).
$$
...
=\mathcal F\left[ \frac{ H_i(T) } { p_i } \right]_k
,
$$
...
G(T)
&\equiv
\sum_{k=1}^{n-1} \tilde \Xi_k^2(T)
=
\sum_{i=1}^{n} \rho_i \, \Xi_{*i}^2(T)
  \notag
  \\
  &=
  \sum_{i=1}^n \rho_i
  ...
  \frac{H_i(T)}{p_i}
  -
  \sum_{j=1}^n \rho_j \frac{ H_j(T) } {p_j}
  \right)^2
.
  \label{eq:hfluc_def}
\end{align}
...
%
\begin{align*}
\bigl\langle \tilde \Xi_k^2(T) \bigr\rangle
&=
\frac{1}{T}
\sum_{t=-\infty}^\infty
\bigl\langle
...
\begin{equation}
  E(T)
  \ge
  \bigl\langle G(T+t_0) \bigr\rangle
  .
  \notag
  %\label{eq:fluc}
\end{equation}
...
$$
G_{<K}(T) \equiv \sum_{k=1}^{K-1} \tilde \Xi_k^2(T),
$$
...
$$
\max_{k=1, \dots, K-1} \left| \, \tilde \Xi_k(T) \, \right|
.
$$
...
\left(
  \sum_{i=1}^n \frac{ \phi_{ki} \, \psi_{li} }{ p_i }
\right)^2
.
\end{align*}
%
In particular, if the eigenvectors, $\pmb \phi_k$
and $\pmb \psi_k$, coincide and $\mathbf p = \pmb \rho$,
we would have
$\bigl\langle
  \tilde \xi_k(t) \, \tilde \xi_k(0)
\bigr\rangle
...
The transition matrix method usually only for
...
%
For a local sampling process,
if we borrow \eqref{eq:lambda_Gaussian}
for $\Lambda_k$,
...
\Gamma_k = \coth\left[
  \left(
    \frac{ \pi \, k \, \sigma_z } { g \, \Delta z}
  \right)^2
  \right].
$$
For small $k$,
we have $\Gamma_k \propto \Delta z^2/(\sigma_z \, k)^2$,\cite{bussi2006}
which means that a doubled sampling range, $\Delta z$,
...
we would expect a roughly flat profile
for the single-bin updating scheme,
...
If the width of the Gaussian updating kernel
is very small compared to $\sigma_\varphi$,
then we can extend the upper limit of the integral
in Eq. \eqref{eq:mu_int} to $\infty$, and
%
\begin{equation}
  \mu(\varphi)
  =
  \frac{            1            }
       { \sqrt{ 2 \pi } \sigma_\varphi }
  %
  \exp\left(
        -\frac{   \varphi^2   }
              { 2 \, \sigma_\varphi^2 }
...
In general, some eigenvalues can turn negative
because of truncation and discretization.
%
We can however maintain the stability of updating scheme
by starting with Eq. \eqref{eq:lambda_Gaussian}
