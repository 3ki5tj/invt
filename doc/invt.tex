...
%
Overly slow and rapid reduction of the updating magnitude
can hurt the precision and accuracy of the bias potential.
%
...
from (cf. Appendix \ref{sec:ave_us}):
\begin{equation}
  \alpha_i = \frac{ 1 - e^{-\lambda_1 \, \hat \alpha_i} } { \lambda_1 }
  ,
  \label{eq:alphai}
\end{equation}
where
...
we will consider here a family of bandpass updating schemes
...
%
However, by constructing a mixture\cite{shirts2017}
or expanded ensemble\cite{swendsen1986,
...
%
where $n^{(i)}(z)$
the number of visits to the bin $(z-\delta/2, z+\delta/2)$
in umbrella $i$,
and the $N^{(i)}$ is the total number of visits
to the umbrella $i$.
%
We can use the final converged value of $Z^{(i)}$
without going through WHAM iteration.
...
both the optimal and inverse-time schedules
were able to reduced the error significantly,
suggesting the potential benefit of using a decreasing updating magnitude
even with a sometimes sub-optimal schedule.
%
It is also reassuring to see that
...
and the former tended to be larger.
%
...
%
When the local noise are dominant,
...
%
For example, the prediction showed that
there was a optimal width
around $\sigma = 0.37$ for $\varrho = 0.1$
or $0.07$ for $\varrho = 0.8$,
assuming the optimal schedule,
and the optimal width also depended on the simulation length
and other parameters.
%
We recall that a wider Gaussian means
more middle-range eigemodes
would have diminishing eigenvalues
...
as the rate of error reduction
is also proportional to the eigenvalue, $\lambda_k$,
by Eq. \eqref{eq:vt_diffeq_mbin}.

An overly large width $\sigma$ can have a dramatic
impact on the initial updating magnitude
...
of the Gaussian updating scheme can be sub-optimal.
%
...
%  \hat u_i = u_i + \frac{ H_i } { \rho_i } -1
%  .
%  \label{eq:vcorr_equil1}
%\end{equation}
%
Using this formula to
the instantaneous histogram, $h_i(t)$,
we get an independent estimate
of the bias potential at a single step $t$,
%
\begin{equation}
  \hat u_i(t)
...
and ignored a constant shift $\alpha(t)$
in the second step.
%
...
${\tilde u}_k(t + 1) = {\tilde u}_k(t) + \alpha(t) \, \lambda_k \, {\tilde f}_k(t)$,
and from Eq. \eqref{eq:u_uhat},
...
an additional constraint on the updating magnitude.
%
...
\begin{align*}
  ...
  &\approx
  \frac{1}{t-t'} \ln \frac{ \Omega(t') } { \Omega(t) }
  =
  \sum_{\tau=t'+1}^t \frac{ \ln[1- \lambda_k \, \alpha(\tau)] } { t - t' }
  \\
  &\approx
  -\lambda_k \cdot \frac{\int_{t'}^t \alpha(\tau) \, d\tau}{t-t'}
  =
  -\lambda_k \cdot \frac{q(t) - q(t')}{t- t'}
  ,
\end{align*}
where $t$ and $t'$ are two consecutive grid points.
%
...
which is not necessarily even to be most efficient.
%
...
that shifts with the bin $j$ of visit
...
Consider an ideal FDS simulation under the exact bias potential.
%
By Eq. \eqref{eq:sh_ave},
the fluctuation of the instantaneous histogram
is then given by the random part,
%
\begin{equation}
  f_{*i}(t) = \xi_{*i}(t)
  ,
